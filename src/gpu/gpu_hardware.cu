...
GpuHardwareInfo query_hardware(int device_id) {
    ...
    size_t mem = 0;
    cudaMemGetInfo(&mem, NULL);
    size_t total = 0;
    cudaError_t e = cudaDeviceGetAttribute((int*)&total, cudaDevAttrTotalGlobalMemory, actual_device);
    info.total_global_memory = (e == cudaSuccess) ? total : mem;

    info.l1_cache_per_sm = (size_t)get_attr(cudaDevAttrL1CacheSize, actual_device);
