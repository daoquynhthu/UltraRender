> **Archive status:** Historical construction roadmap frozen on 2026-08-01. It does not define the current cursor; use the root `PLAN.md`.

# UltraRender 升级路线图 (PLAN.md)

最后更新: 2026-08-01 (Phase U closure and Phase X cursor)
...
- **Phase U.4 已闭环**: 实际 `HdMaterial` SPrim 归一化 legacy/new Hydra network，将 URE adapter nodes、bounded `UsdPreviewSurface`、UV texture/primvar set 映射为不可变 MaterialGraph；accepted/rejected 网络均保留结构化 loss report，未知 connected semantic fail-closed。
- **Phase U.5 已闭环**: 实际 `HdRenderPass`、`HdRenderBuffer` 和 camera SPrim 将 retained Hydra mesh/material snapshot 降低到 SceneIR，并由 `RenderSession` 管理 CUDA 上下文、逐次执行 progressive pass、AOV 写回、相机重置、收敛与 fail-loud 诊断；不创建 OpenGL/Vulkan/window context。
- **Phase U 已闭环**: U.1-U.6 已完成 native schema mapping、实际 OpenUSD Hydra delegate、mesh/material/session bridge 和严格 native-to-USDA adapter；有损输出必须携带结构化 report，不可表达语义 fail-loud。
- **当前唯一施工项 — Phase X**: 只在稳定 Session/MaterialGraph/native scene contracts 上暴露插件 ABI。
...
**背景**:
[Phase E 光谱架构设计文档](../Phase_E_Spectral_Architecture.md) 定义了完整的架构决策。
...
**核心审计结论**: W.0 初始审计确认 production `RayQueue` 只传输 spectral throughput + wavelength PDF + Stokes (`I,Q,U,V`)，局部 `ComplexF` 不构成跨路径相干状态。Phase W 随后建立了 host/CUDA reference complex/Jones、OPL、传播、部分相干和 coherent distributed sufficient-statistics contracts，但没有把它们伪装为 scene-integrated production coherent queue/film。因此系统能处理已验证的局部边界、显式 diffraction camera、radiometric diffractive/fluorescence 路径和受限参考问题，仍不能普遍处理任意场景的多路径相干、全息或全局 Maxwell 求解。详见 `docs/Phase_W_Wave_Optics_Audit.md`。
...
所有高级波动特性由 `WaveOpticsConfig` 控制，并一致暴露到 `ure::RenderConfig`、JSON、CLI、C ABI、Session API、`pyure` 和 distributed shard metadata。材质或场景可以声明需要某能力，但是否启用由 render config 决定。
...
| W.1 | 配置/API 骨架：新增 `WaveOpticsConfig`，贯穿 `ure::RenderConfig`、JSON、CLI、C ABI、Session API、pyure；默认全关；unsupported feature 在 scene load/session create 前 fail-loud | 已完成：JSON/CLI/C ABI/pyure 已覆盖；未知 mode fail-loud；非 radiometric solver/feature 在实现前拒绝创建或运行；`test_config`、`test_session`、pyure smoke 与 CLI invalid-mode gate 通过。Distributed coherent shard metadata 已由 W.11 补齐 |
...
| W.3 | Coherent field state：定义 `ComplexSpectrum`、Jones field、OPL/phase accumulation、coherence group/source coherence metadata、coherent realization id；新增 `ComplexFieldFilm` 和 coherent accumulation order | 已完成基础合约闭环：`ComplexSpectrum` / `JonesSpectrum` / `CoherenceMetadata` / OPL phase helpers / `ComplexFieldFilm` 已进入 `ure::wave` host oracle 层；测试覆盖 OPL 相位推进、Jones 分量功率保持、同相两束 `|sum E|^2 = 4`、反相抵消为 0、incoherent `sum |E|^2 = 2` 与 out-of-range/invalid fail-closed。W.7 已补齐 partial-coherence generalized-ray/CSD reference 与 CUDA ensemble reduction，W.11 已补齐 serialized distributed sufficient-statistics merge；主 GPU path tracer 的 scene-integrated coherent field transport 仍明确不可用 |
...
| W.11 | Coherent distributed contract：distributed range/RGB metadata 升级到 v6，以 `DistributedFrameSemantics` 严格区分 radiance、complex field、mutual intensity 与 coherent realization；非 radiance payload 必须绑定 phase-reference、field-layout、source/group/realization identity。Complex frame 以逐元素 complex estimator sum + weight 在 normalization 前事务合并；coherent realization 必须先完成 field merge，再进入 W.7 amplitude sum → realization power → weighted realization average → incoherent group sum。Mutual-intensity frame 保存 weighted Hermitian PSD CSD、total statistical weight 与有序不重叠 realization ranges，合并后才归一化。独立 field/CSD magic 带 version、byte-order marker、全文件 SHA-256 footer、资源/worker/compiler/executable/cache/sample provenance，拒绝 corruption/trailing/cross-kind/overlap/layout/phase mismatch；v4/v5 radiance 只读兼容。RGB writer/merge 只接受 radiance，coherent schedule 不能使用默认 semantics。Release 56/56、SDK-free public surface、W.11/T/Q/V/physics-optics/documentation gates 通过；production renderer 仍不输出 coherent frame | 已完成 |
| W.12 | 统一验证和静态审计：`run_phase_w_validation_suite.ps1` 构建 Release、冻结 required CTest inventory、运行 W.2/W.5/W.6/W.7/W.9/W.10/W.11/W.12/T/documentation/physics-optics gates、执行完整 CTest，并输出带 source/artifact SHA-256 的 `ure.phase_w.validation.v1`。证据矩阵覆盖 Airy first zero、slit/grating angles、two-beam interference、独立 complex Airy thin-film phase、rough dielectric spectral/UV PDF、Stokes/Jones coherency transform、fluorescence Stokes shift、energy/PDF、coherent-before-incoherent distributed merge、unsupported fail-loud 和 JSON/CLI/C/native/Python parity。静态门禁阻断 complex/CSD sufficient statistics 进入 RGB output path。Release 56/56 通过；Phase W 在既定 production/reference 边界内完成，production coherent session/worker 仍 fail-loud | 已完成 |
...
| U.4 | ✅ 材质转换：实际 `HdMaterial` legacy/new network 中的 USD `Material` + URE adapter nodes → immutable URE MaterialGraph；能力损失输出 structured loss report，未知 connected semantic fail-closed | 中 |
| U.5 | ✅ 交互渲染：实际 Hydra camera/render-buffer/render-pass → retained SceneIR → CUDA `RenderSession` 渐进执行；Beauty/normal/depth/albedo/UV/motion AOV、相机重置、收敛和错误统计均有实际 OpenUSD+GPU 门禁，且不依赖图形/窗口上下文 | 高 |
| U.6 | ✅ 场景导出：validated native archive、`.ure/.urescene` 与显式选取的 `.urepkg` scene → deterministic `.usda`；严格模式拒绝任何损失，有损模式强制持久化 loss report，不可表达能力 fail-loud | 低 |
