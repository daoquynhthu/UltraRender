...
Scene tooling commands use the runtime's exact-build Scene Tool 0.1 extension so that `validate`, `inspect`, `build`, `migrate`, `pack`, `unpack` and `realize` share one capability registry and scene realizer; `render` then consumes the resulting source through ProductJob 0.3. CLI filesystem argument handling and terminal presentation remain local, but semantic validation, migration, realization and package publication have one implementation. The separate native tool retains only adapter export and does not duplicate these policies.
...
Its result is an immutable `ProductSnapshot` containing a deep-frozen realized scene, resolved resource set, execution constraints, bounded time state and content identities. Renderer consumers receive a deep copy rather than mutable access to retained snapshot storage. Required unsupported semantics reject before renderer allocation. Tooling-only optional data is retained in diagnostics and provenance, never silently discarded. Material-program convergence beyond the currently realized SceneIR subset remains PRV.3.
