...
The first Preview extension began as ProductJob 0.1 and is now exact-build ProductJob 0.2. It reuses Core handles, operations and immutable Frame leases while exposing product build/snapshot/objective/plan identities, separate requested/accepted/completed work and an artifact manifest. It does not add fields or functions to any Core 1.0 prefix. One job/session is serial; the maintained Worker client uses an explicit bounded cross-session capacity and reports excess concurrency as `Backpressure`. Until later Preview phases define richer execution, unsupported determinism, usage, output, latency and precision-losing budget requests reject explicitly.
