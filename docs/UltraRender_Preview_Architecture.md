...
The completed PRV.3 boundary is deliberately evidence-scoped. Native MaterialGraph programs and accepted packaged material resources enter the Product Realizer path. Bounded glTF and MaterialX/preset inputs convert to canonical native artifacts, and retained Direct/Worker ProductJob images plus CLI authoring parity now close those declared subsets. Hydra-derived material conversion produces a canonical artifact, but the Hydra viewport remains a legacy adapter path rather than a ProductJob client until PRV.10. Bounded radiometric diffraction, fluorescence and Mie routes are supported only for their validated applicability, resource and color-artifact contracts. They do not establish a general coherent-wave or volume promise; typed transport/detector wavelength, Stokes and lifetime planes remain PRV.4 work, and closure stays tied to the retained scenarios recorded in the ledgers.
