...
PRV.1 implemented this spine for the bounded native-scene color workflow. `ure_client` supplies explicit Direct and Worker transports, and CLI render uses only this client and defaults to Worker. PRV.1R evolved the exact-build ProductJob extension from 0.1 to 0.3 while leaving Core ABI 1.0 unchanged; PRV.3 advances that unstable extension to ProductJob 0.4. ProductJob 0.4 adds observational eligible, qualified and executed integrator masks; these are extension data, not Core ABI fields. PRV.1R closed canonical work accounting, persistent execution, asynchronous Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive feedback, latest immutable frames, generic multi-plane leases and the renderer-free external C++ package. PRV.4 adds exact-build MeasurementFrame and Output interfaces over the same product session without expanding Core ABI 1.0. Retained 480p functional and 720p/1080p quality evidence establishes ProductE2E for the bounded implemented paths; the old smoke evidence remains only routing/transport history.

PRV.2 implements complete archive realization for the bounded supported subset. Native, packaged and adapted archives enter one Scene Realizer, which executes deterministic procedural graphs, resolves content-addressed resources under explicit budgets, compiles solver applicability and bounded time constraints, and produces an immutable `ProductSnapshot`. Self-contained `.urepkg` inputs share this authority across Direct, Worker and CLI. A retained 854×480 procedural-package render closes ProductE2E for that path after removal of the author resource directory. PRV.3 advances the exact-build Scene Tool to 0.2, adds canonical material operations and preflight of accepted material resources, and retains actual ProductJob images for bounded glTF, preset and MaterialX-derived artifacts rather than relying on adapter component tests. PRV.4 supplies typed complete-scene measurements and official artifact publication; reconstruction and denoising remain subsequent work.

The bounded automatic bridge may choose among unbiased complete candidate techniques. Product memory planning therefore selects the largest candidate subset that fits the pre-allocation budget and binds that selection, executor count and memory plan into the Product plan identity. This is plan selection, not an implicit capability fallback: spectral lanes, requested work, precision, output semantics and reconstruction requirements cannot be reduced. If no applicable unbiased plan remains, submission fails before renderer allocation.
...
Scene tooling commands use the runtime's exact-build Scene Tool 0.2 extension so that `validate`, `inspect`, `build`, `migrate`, `pack`, `unpack`, `realize` and bounded material operations share one capability registry and scene realizer; `render` then consumes the resulting source through ProductJob 0.4. CLI filesystem argument handling and terminal presentation remain local, but semantic validation, migration, realization and package publication have one implementation. The separate native tool retains only adapter export and does not duplicate these policies.
...
The first Preview extension began as ProductJob 0.1 and is now exact-build ProductJob 0.4. It reuses Core handles, operations and immutable Frame leases while exposing product build/snapshot/objective/plan identities, separate requested/accepted/completed work, monotonic progressive state, an artifact manifest and the eligible/qualified/executed integrator observations. Scene Tool 0.2 is a separate exact-build extension for bounded scene and material operations and their structured results. Neither extension adds fields or functions to any Core 1.0 prefix. One job/session is serial; the maintained Worker client uses an explicit bounded cross-session capacity and reports excess concurrency as `Backpressure`. Until later Preview phases define richer execution, unsupported determinism, usage, output, latency and precision-losing budget requests reject explicitly.
...
Its result is an immutable `ProductSnapshot` containing a deep-frozen realized scene, resolved resource set, execution constraints, bounded time state and content identities. Renderer consumers receive a deep copy rather than mutable access to retained snapshot storage. Required unsupported semantics reject before renderer allocation. Accepted material resources are also preflighted here: texture decoding, SPD validity, Mie phase resources and medium constraints fail loudly before renderer allocation and are never replaced by an implicit default. Tooling-only optional data is retained in diagnostics and provenance, never silently discarded. The canonical material program set is the product authority: MaterialGraph supplies graph topology, while every complete-scene material field still consumed by backend lowering is included in the same immutable identity. Adapter provenance is canonicalized into that set where supported, while adapter-derived ProductE2E evidence is tracked separately.

The completed PRV.3 boundary is deliberately evidence-scoped. Native MaterialGraph programs and accepted packaged material resources enter the Product Realizer path. Bounded glTF and MaterialX/preset inputs convert to canonical native artifacts, and retained Direct/Worker ProductJob images plus CLI authoring parity close those declared subsets. Hydra-derived material conversion produces a canonical artifact, but the Hydra viewport remains a legacy adapter path rather than a ProductJob client until PRV.10. Bounded radiometric diffraction, fluorescence and Mie routes are supported only for their validated applicability, resource and color-artifact contracts. PRV.4 exposes the measurements these implemented producers actually supply; it explicitly rejects Spectrum/Stokes requests that lack a complete-scene producer and does not establish a general coherent-wave or volume promise. Closure stays tied to the retained scenarios recorded in the ledgers.

`.urepkg` Preview packages are self-contained for every required local resource. Ambient search paths, current directories and author-machine absolute paths cannot be necessary for execution. Packaged payloads are deduplicated by content identity; optional caches are rebuildable and excluded from source semantic identity. Explicit external provider references require a versioned capability and are not treated as packaged resources.
...
PRV.4 implements this boundary for the current bounded complete-scene CUDA profile. A completed ProductJob publishes 45 BeautyRaw, geometry AOV, sufficient-statistics and estimator planes; each plane carries observable, unit, measure, time, uncertainty, provenance, endpoint and sample-range metadata. Exact-build Direct and Worker clients discover the same immutable plane set and may copy arbitrary byte ranges without acquiring the entire plane. Core-only clients continue to receive the frozen single Color fallback. Spectrum and Stokes remain capability-gated: a request is rejected at job compilation when no real complete-scene producer exists.
...
The PRV.4 implementation uses official OpenEXR 3.4.14 and Imath 3.2.2 as static runtime dependencies. This preserves the stable Core loader's single-DLL behavior while the runtime package retains both third-party licenses. The published EXR is a flat multi-channel image, not deep EXR; the versioned measurement checkpoint remains authoritative for statistics that EXR cannot represent. CLI delegates publication to the same Output extension used by `ure_client` rather than encoding an alternate output.
