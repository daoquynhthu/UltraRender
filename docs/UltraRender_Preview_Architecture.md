...
PRV.1 implemented this spine for the bounded native-scene color workflow. `ure_client` supplies explicit Direct and Worker transports, and CLI render uses only this client and defaults to Worker. PRV.1R evolves the exact-build ProductJob extension from 0.1 to 0.3 while leaving Core ABI 1.0 unchanged. The common diagnostic envelope, sample precedence, Device/Execution 0.1 semantics, monotonic progressive feedback, latest immutable frames and renderer-free external C++ package are implemented, but the retained smoke evidence still proves only routing and transport structure. PRV.1R must close tiered image validation before complete archive realization begins. Complete archive realization, scene tooling through the runtime, typed measurements, reconstruction and official artifact publication remain subsequent PRV phases.
