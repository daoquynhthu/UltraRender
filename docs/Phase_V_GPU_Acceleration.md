...
| V0-TRV | Closest-hit and shadow traversal each use `int stack[64]`, push both children without near/far ordering and expose no overflow metric. The existing `stack_ptr < 64` guard can still write the second child out of bounds when one slot remains; deeper work can otherwise be dropped silently. | Memory safety and missed-hit correctness are not fail-loud for deep or adversarial trees. | V.2 |
| V0-INS | Closest-hit scans every instance and then traverses its mesh BVH. The separate shadow `any_hit` path scans spheres and non-instanced meshes but does not traverse `scene.instance_count`. | No TLAS exists, instance cost is linear, and instanced geometry can be absent from the current shadow shortcut. | V.2/V.3 |
