...
PRV.2 productizes native tooling through the exact-build runtime Scene Tool 0.1 `UnstableExtension`. `ure_cli validate/inspect/build/migrate/pack/unpack/realize` uses `ure_client` with an explicit Direct or Worker transport; it does not own validation, migration, package or realization policy. Validation and realization report schema compatibility, feature disposition, content-resolved resource inventory, stored/decompressed/resident/streamed/temporary/output budgets and ProductSnapshot identity. `ultrarender_native_tool` now retains only adapter export. The older C session and pyure loaders remain legacy experimental surfaces pending PRV.10 and are not an alternative authority.
