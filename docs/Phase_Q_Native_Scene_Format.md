...
PRV.2 productized native tooling through the exact-build runtime Scene Tool 0.1 `UnstableExtension`; PRV.3 advanced it to 0.2 with bounded material operations. `ure_cli validate/inspect/build/migrate/pack/unpack/realize/material-*` uses `ure_client` with an explicit Direct or Worker transport; it does not own validation, migration, package, realization or material policy. Validation and realization report schema compatibility, feature disposition, content-resolved resource inventory, stored/decompressed/resident/streamed/temporary/output budgets, canonical material identity and ProductSnapshot identity. `ultrarender_native_tool` now retains only adapter export. The older C session and pyure loaders remain legacy experimental surfaces pending PRV.10 and are not an alternative authority.

PRV.2 packages embed every required local resource payload, provenance and optional rebuildable cache data. Package execution ignores ambient CWD, undeclared search paths and author-machine absolute paths. Pack/unpack/build/migrate preserve canonical scene and resource identities; cache removal does not alter the source/package semantic identity. Package dependencies that would make execution non-self-contained are rejected by the current bounded product path.
...
Compiled caches use the separate `UREC` binary identity and remain disposable. Their manifest binds source, compiler, schema, and SceneIR hashes to GPU upload ranges, spectral/resource artifacts, acceleration metadata, and validation metrics. A mismatch either requests an explicit rebuild or is rejected according to caller policy. PRV.2 may preserve an optional cache inside `.urepkg`, but ProductSnapshot realization does not depend on it and deleting it leaves the semantic identity unchanged; complete product cache lifecycle remains PRV.9. Farm scheduling consumes package resource hashes and worker inventories, applies capacity gates, and deterministically minimizes resource transfer.
