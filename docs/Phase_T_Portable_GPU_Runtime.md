...
T.0 through T.8 are complete and the authoritative cursor is T.9. This document
is the migration ledger for T.9 through T.11. CUDA remains the complete scene
rendering backend. Vulkan has a production compute-runtime foundation and a
bounded acceleration bridge on Windows and Linux, but full SceneIR rendering is
not yet lowered to it. D3D12 remains reserved.
...
| T0-DEV | Backend-neutral adapter/capability identity owns public device semantics. CUDA and Vulkan enumeration stay behind backend-private registration; Vulkan inventory records native ray-query/ray-pipeline hardware capability only when the required extension and feature chain is present, while providers advertise only executable paths. | Adapter/capability registry | T.1/T.6-T.8 complete |
...
| T0-KRN | Estimator/PDF versions and critical stage order are backend-neutral. Existing optimized `.cu` bodies remain a private CUDA fast path; Vulkan foundation and acceleration SPIR-V are generated by the pinned Slang toolchain. | Kernel toolchain and semantic library | T.2/T.5-T.8 complete; T.9 additional lowering |
...
| T0-WAVE | Fraunhofer CUDA execution and the Vulkan reference propagation operator consume the shared wave semantic and runtime-owned resources/timelines. | Wave operator/runtime integration | T.2/T.5-T.7 complete; T.9 additional backend |
| T0-ACC | SDK-free acceleration descriptors, selection policy, stable ray/hit layout and provider lifetime now bridge a bounded triangle BLAS/TLAS to Vulkan ray query. Compute fallback and reject semantics are explicit. CUDA `world_hit` remains the production reference; `gpu_accelerator.hpp::OptixAccelerator` is still a nonfunctional stub. | Acceleration-provider API | T.8 bridge complete; production construction/refit/compaction remains Phase V |
...
| T0-TEST | SDK-free gates cover runtime/resources/execution/acceleration and the installed public surface. CUDA production traversal and Vulkan native/fallback traversal share exact hit fixtures; Vulkan executes on Windows NVIDIA/Intel and Linux. Full renderer parity remains T.11. | Validation architecture | T.1-T.8 complete; T.11 full parity |
...
9. T.9 through T.11 add optional D3D12, scheduling, and full
...
- Vulkan adapters are discoverable and report their acceleration capabilities.
  Full render selection remains rejected until complete SceneIR lowering exists.
  D3D12 is unavailable.
...
remain fail-loud. The authoritative cursor is T.9.
