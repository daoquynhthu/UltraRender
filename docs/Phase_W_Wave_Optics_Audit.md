...
|----------|-----|----------|----------------|
| P0 | No coherent field state | `RayQueue` carries scalar throughput per wavelength plus Stokes components, but no Jones field, complex spectrum, optical path length, phase, coherence id, or source coherence metadata. | Cannot compute two-slit interference, multipath interference, speckle, holography, phase-sensitive sensing, or long-distance optical-path phase accumulation. |
| P0 | Film/accumulation is radiance-only | Accumulation paths in `path_tracer_wavefront.cuh` atomic-add RGB/XYZ-derived values to `GpuVec3* accum_buffer`; distributed merge sums float framebuffer values in `distributed_contract.cpp`. | Coherent imaging requires `|sum E|^2` within a coherent realization, then incoherent averaging across realizations. Current merge order is mathematically wrong for coherent fields. |
...
Phase W should be a complete wave-optics solver track, not a thin-film cleanup. The first deliverable should be a feature-gated solver boundary and a diffraction camera path because it gives visible, analytic, low-intrusion validation. W.2 has started with a `WaveFieldGrid` complex field carrier, direct Fraunhofer/Fresnel/angular-spectrum CPU propagation oracles, circular-aperture Airy PSF, diffraction-limited MTF, defocused circular-pupil host oracle, and feature-gated `DiffractionCameraPlan` in `ure::wave`; this gives the GPU diffraction camera a hard reference for first-zero radius, sensor-space scaling, encircled energy, wavelength scaling, radial symmetry, normalized discrete PSF kernels, cutoff frequency, MTF monotonicity, pupil aperture masking, defocus phase, sampled complex-field power, propagation normalization, and half-enabled config rejection. Direct `GpuRenderEngine` scene load now also rejects camera diffraction requests before GPU initialization, so direct C++ users cannot bypass CLI/Session gating and silently run the radiometric path. Full coherent path transport, partial coherence, diffractive materials, and local full-wave coupling should follow after the film/merge/API contracts are capable of preserving complex-field semantics.
