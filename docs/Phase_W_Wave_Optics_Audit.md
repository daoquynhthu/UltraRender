...
Phase W should be a complete wave-optics solver track, not a thin-film cleanup. The first deliverable should be a feature-gated solver boundary and a diffraction camera path because it gives visible, analytic, low-intrusion validation. W.2 has started with a circular-aperture Airy PSF host oracle in `ure::wave`; this gives the GPU diffraction camera a hard reference for first-zero radius, sensor-space scaling, encircled energy, wavelength scaling, and radial symmetry. Full coherent path transport, partial coherence, diffractive materials, and local full-wave coupling should follow after the film/merge/API contracts are capable of preserving complex-field semantics.
