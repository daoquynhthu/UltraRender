...
Initial targets and reciprocal trials share one normalized seed proposal over a GPU-resident geometry catalog. The catalog contains every non-degenerate sphere, direct triangle, and transformed instance triangle with stable geometry identity and UV data. It deliberately does not filter by current material: material and texture mutation can change smooth-specular eligibility without changing topology, and such ineligible proposals remain ordinary Bernoulli misses. A discrete catalog sample is followed by uniform-area sphere or triangle parameter sampling. Event count, primitive sequence, reflection/transmission branch, and local parameters are drawn by the same proposal for target and trial so the reciprocal estimate includes topology and convergence-basin probability rather than conditioning on the old camera trailing chain.
