...
The immutable solution separates the Newton constraint determinant from radiometric geometry. It reconstructs the endpoint area Jacobian and generalized geometry `|P2 A^-1 B_L| G(anchor, first)`, persists catalog/geometry/primitive/material identity and reflection/transmission topology for every solved event, validates every reconstructed segment with production traversal, and reports TIR, singular, stale, occluded, invalid-differential, non-delta, spectral-split, or out-of-domain failures through typed telemetry. Response and replay consume this immutable identity instead of indexing the historical camera delta seed.
...
The SDS response evaluator is a separate artifact-producing kernel. It reevaluates light emission expressions and textures at the sampled light UV, transports a spectral Stokes packet through the solved chain with the shared Mueller boundary implementation in radiance mode, evaluates the non-delta anchor BSDF, and composes those factors with the generalized geometry, light endpoint PDF, explicit reciprocal-root weight, and MIS weight. Each immutable event retains the solved incident/transmitted IOR pair, so dielectric reflection and transmission replay use the actual surrounding medium instead of an air-only approximation. The artifact retains emission, anchor scattering, specular response, and final Stokes components for replay and audit.

Root selection follows reciprocal-probability SMS rather than a fixed trial cap. Counter-based IID dimensions drive target and trial walks; the renderer batches eight independent trials per bounded GPU pass and continues until the target root is reproduced. Root identity includes the selected camera anchor, event count, catalog/geometry/primitive sequence, reflection/transmission branches, and converged world-space positions. The geometric trial count is the unbiased estimator of reciprocal root probability. A render sample becomes complete only after all valid targets resolve, and the host scheduler drains pending states without a maximum pass count before conversion and film commit. A fixed maximum trial count, low-discrepancy correlated trials, treating a topology/root probability as one, or committing censored pending trials is forbidden because each introduces bias.

Initial targets and reciprocal trials share one normalized seed proposal over a GPU-resident geometry catalog. The catalog contains every non-degenerate sphere, direct triangle, and transformed instance triangle with stable geometry identity and UV data. It deliberately does not filter by current material: material and texture mutation can change smooth-specular eligibility without changing topology, and such ineligible proposals remain ordinary Bernoulli misses. A discrete catalog sample is followed by uniform-area sphere or triangle parameter sampling. Camera anchor, event count, primitive sequence, reflection/transmission branch, and local parameters are drawn by the same proposal for target and trial so the reciprocal estimate includes strategy, topology, and convergence-basin probability rather than conditioning on the old camera trailing chain. The immutable artifact records the anchor, event-count, surface-area, catalog, and branch densities for audit; they are not divided into the contribution separately because the reciprocal basin estimator already integrates the complete normalized primary-space proposal.

The active BDPT/VCM connection and merge kernels reject any bridge endpoint that is delta and cannot traverse a hidden interior delta chain. An SMS solution containing at least one interior smooth-specular event therefore has disjoint support from those active techniques and receives an explicit support-exclusive MIS weight of one. Ordinary connection and merge paths retain their complete-path power-heuristic denominator. The resolved SDS artifact is converted with the anchor wavelength state and enters the same single bidirectional film commit; it is never added through a second estimator path.
