...
This index separates current authorities, completed subsystem references, and historical records. The root `PLAN.md` is the only authoritative global execution queue; its current cursor is PRV.1. `README.md` and `STATUS.md` summarize the current user-visible state; source code, CMake/CTest registration and fresh command output remain the strongest implementation evidence.
...
| [`../PLAN.md`](../PLAN.md) | Authoritative `UltraRender_preview` integration queue; PRV.1 is current |
