...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U 与 HO.0-HO.2 已完成。HO.1 的 SDK-free `ure_transport` 冻结 typed observable、measure/support、estimator、单位/时间、provenance、uncertainty 和 compatibility；HO.2 的 `ure_research` 提供确定性研究分片、可选择读取的 measurement artifact、replicate comparison、能力协商、bounded oracle 和晋级证据检查。当前权威游标是 `HT.0 — 现有积分器描述化`。这些合同不表示现有积分器已完成自动组合，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 59 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
├── libs/ure_transport/ # observable、measure、estimator、uncertainty 与 compatibility contracts
...
当前构建树注册 59 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
