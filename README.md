...
- Phase Q 与 Phase R 已完成；Phase T 已完成 CUDA production backend 向 backend-neutral runtime/resource/execution contract 的迁移，当前施工游标是 `T.7`（Vulkan compute production foundation）。
...
- Slang 2026.14 已用六类真实计算原型完成固定版本、确定性多目标编译、反射、debug mapping、CUDA 占用率及数值执行验证；现有 production kernels 仍是私有 `.cu` fast path，后续 backend 的共享 Slang lowering 尚未实现，Slang RHI 也未被引入。
...
当前构建树注册 41 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
