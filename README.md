...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2、HT.0-HT.3 与 HR.0 已完成。SDK-free `ure_transport` 现在提供 bounded support/measure composition、pilot 资格判定，以及绑定 tile/wavelength/time/device/sample/chain 的 cost-aware portfolio 调度、漂移 re-pilot 和分布式精确覆盖合同；SDK-free `ure_reconstruction` 提供 typed MeasurementBundle、schedule provenance 与 versioned checkpoint。现有 CUDA 路径尚未并发执行全部 technique，也尚未生产所有高阶 plane。当前权威游标是 `HT.4 — 新积分器与 proposal 研究平台`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 66 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
当前构建树注册 66 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
