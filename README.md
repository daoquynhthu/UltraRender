...
- Phase Q、Phase R、Phase T、Phase V 与声明范围内的 Phase W 已完成。Phase W.2 的 diffraction camera、W.5 的 diffractive MaterialGraph operators 与 W.6 的 fluorescence material 已接入显式启用的 CUDA wavefront 路径；W.7、W.9、W.10 与 W.11 分别建立部分相干统计、各向异性模态、局部全波交换和相干分布式充分统计合同，W.12 以统一物理/API/fail-loud/静态门禁完成闭环。当前施工游标是 `U.1`。
...
- 当前 Release 构建注册 56 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
