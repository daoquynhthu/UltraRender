...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2、HT.0-HT.2 与 HR.0 已完成。SDK-free `ure_transport` 现在提供 bounded path-event grammar、精确 support partition、canonical-measure transform、分层 MIS/GRIS/MCMC 组合，以及绑定 world/snapshot/support 的 pilot 统计和自动资格判定；SDK-free `ure_reconstruction` 提供 typed MeasurementBundle 与 merge-safe checkpoint。现有 CUDA 路径尚未并发执行全部 technique，也尚未生产所有高阶 plane。当前权威游标是 `HT.3 — 在线 portfolio 调度`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 65 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
├── libs/ure_transport/ # transport semantics、Technique Graph、composition、pilot 与 qualification contracts
├── libs/ure_research/  # research manifest、artifact、comparison、capability、oracle 与 promotion contracts
├── libs/ure_reconstruction/ # typed MeasurementBundle、statistics、merge 与 checkpoint contracts
...
当前构建树注册 65 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
