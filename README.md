...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2、HT.0-HT.5 与 HR.0-HR.1 已完成。SDK-free transport 层可形成带 coverage、normalization、uncertainty 和预算身份的自动计划；统计重建层保留 raw estimate，在 spectral/Stokes 物理域提供 variance/tail-aware spatial-temporal baseline 与逐像素置信、support、rejection provenance。当前 CUDA 完整场景路径尚未生产全部高阶 plane，因此不会把该基线伪装成无条件默认降噪。当前权威游标是 `HR.2 — Sample-level 与光谱/偏振重建`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 70 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
├── libs/ure_reconstruction/ # typed MeasurementBundle、statistics、reconstruction、merge 与 checkpoint contracts
