...
- PB.0-PB.4 已完成公共交互面分类、Candidate registry/生成式 SDK、Windows x64 两导出 loader ABI、Core 生命周期、immutable frame lease 与本地 worker 基础。`ure_worker` 通过 same-user Named Pipe 和只读 shared-memory lease 传递有界 frame，并且只经两个 loader symbol 装载 product runtime；测试专用 frame producer 不进入候选包。这里仍没有 public native-scene/session/render workflow，也不是稳定 ABI/Protocol。当前权威游标是 `PB.5 — Native full-scene boundary and revisioned replacement`，HR.3 暂缓。架构与施工细则见 [公共 API/ABI 架构](docs/Public_API_ABI_Architecture.md) 和 [Phase PB 计划](docs/PB_Public_Boundary_PLAN.md)；原 Phase X 通用插件生态继续冻结。
...
- 当前 Release 构建注册 89 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
