...
- PB.0-PB.1 已完成公共交互面分类、legacy C/DLL 基线，以及单一 Candidate 0.1 contract registry、确定性 codegen、C11 headers、FlatBuffers schemas、golden messages 和 renderer-free mock frontend kit。外部客户端现在可依赖自包含 staging package 开始并行集成，但这不是可发布的稳定 ABI/Protocol，也没有真实 runtime DLL。当前权威游标是 `PB.2 — Windows x64 loader ABI and candidate runtime product`，HR.3 暂缓。架构与施工细则见 [公共 API/ABI 架构](docs/Public_API_ABI_Architecture.md) 和 [Phase PB 计划](docs/PB_Public_Boundary_PLAN.md)；原 Phase X 通用插件生态继续冻结。
...
- 当前 Release 构建注册 81 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
