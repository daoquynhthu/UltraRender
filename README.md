...
- Phase Q、Phase R、Phase T 与 Phase V 已完成。Phase W.2 的 diffraction camera 与 W.5 的 diffractive MaterialGraph operators 已接入显式启用的 CUDA wavefront 路径；当前施工游标是 `W.6`。
...
- 显式启用的 camera diffraction 支持圆孔或规则叶片 pupil、defocus phase、sensor-pixel integration 与 wavelength-binned PSF resolve。独立的 diffractive-material gate 支持 grating、sinusoidal phase mask、ideal zone plate、blazed DOE 与有界 RCWA/FMM Jones table，并同样限制在普通 CUDA wavefront 积分器。coherent field、partial coherence、fluorescence 和局部全波求解仍属于 Phase W 后续工作；不支持的组合不会静默近似。
