...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2、HT.0-HT.4 与 HR.0 已完成。SDK-free transport/research 层现在提供 bounded composition、pilot/portfolio 调度，以及 capsule-bound `ResearchExtension`、joint-sample/reuse validity、显式 opt-in graph materialization 和 replicated assessment；一个解析 control-variate 正结果与一个 MCMC covariance 负结果保留为 Research 证据，不代表新增生产积分器。现有 CUDA 路径尚未并发执行全部 technique，也尚未生产所有高阶 plane。当前权威游标是 `HT.5 — 自动积分系统闭环`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 67 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
当前构建树注册 67 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
