...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2、HT.0 与 HR.0 已完成。SDK-free `ure_reconstruction` 现在提供 typed MeasurementBundle、预算降级记录、merge-safe sufficient statistics 与可局部读取的 checkpoint；现有 CUDA 路径尚未生产所有高阶 plane。当前权威游标是 `HT.1 — Support/Measure Graph 与组合器`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 61 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
