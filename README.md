...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U 与 HO.0-HO.1 已完成。HO.0 建立可执行能力/研究基线；HO.1 新增 SDK-free `ure_transport`，以 typed observable、canonical measure/support、estimator、单位/时间、provenance、uncertainty 和五态 compatibility algebra 阻止 RGB、MCMC、coherent 与不同 world snapshot 被错误混合。当前权威游标是 `HO.2 — 可执行研究底座`。原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 58 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
当前构建树注册 58 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
