...
- Phase Q、Phase R、Phase T、Phase V、声明范围内的 Phase W、Phase U、HO.0-HO.2 与 HT.0 已完成。`ure_transport` 现在以 SDK-free Technique Graph 描述 wavefront、guiding、ReSTIR、SMS、BDPT、VCM 和 PSSMLT，并将旧配置编译为保持现有行为的 legacy preset；这不表示技术已经自动组合。当前权威游标是 `HR.0 — MeasurementBundle / Feature Film`，原 Phase X 插件 ABI 继续冻结。
...
- 当前 Release 构建注册 60 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
...
当前构建树注册 60 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
