...
- Phase Q、Phase R 与 Phase T 已完成。Phase V 的 V.0 acceleration audit 已冻结当前 mesh-local BVH、fixed stack、linear fallback、无 TLAS/BLAS 和 legacy host/OptiX stub 风险；当前施工游标是 `V.1`（backend-neutral `AccelerationConfig` API）。
...
- 当前 CUDA self-compute acceleration 仍是每 mesh binary BVH 加 instance linear scan；固定 traversal stack、shadow instance coverage、silent triangle fallback、TLAS/refit、build quality/stats 和 native provider productionization 是 Phase V 的显式施工项，不能由现有 OptiX placeholder 或 bounded Vulkan/DXR fixture 代替。
...
当前构建树注册 46 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
