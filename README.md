...
- PB.0 已完成 25 个公共/历史交互面的完整分类、14 个唯一权威域、legacy C header/DLL export 基线、可执行 C11 migration client 和确定性负例审计。当前权威游标是 `PB.1 — Contract registry, generation, mock worker, and frontend kit`，HR.3 暂缓。Phase PB 将继续建立单一 contract registry、Windows x64 C ABI 和本地隔离 worker，使外部前端可独立开发；PB.0-PB.7 仅为 Candidate 0.x，不构成稳定兼容承诺。架构与施工细则见 [公共 API/ABI 架构](docs/Public_API_ABI_Architecture.md) 和 [Phase PB 计划](docs/PB_Public_Boundary_PLAN.md)。原 Phase X 通用插件生态继续冻结。
...
- 当前 Release 构建注册 72 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
