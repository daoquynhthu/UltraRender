...
- Phase Q、Phase R 与 Phase T 已完成。Phase V 的 V.0-V.8 已完成；当前施工游标是 `V.9`，负责按物理误差选择 clustered geometry LoD。
...
当前构建树注册 50 个 CTest。这个数字是当前快照，不应用作长期固定接口；以 `ctest -N` 的输出为准。
