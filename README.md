...
- PB.0-PB.3 已完成公共交互面分类、legacy 基线、单一 Candidate registry/生成式 SDK/mock kit、Windows x64 两导出 loader ABI，以及最小 Core 生命周期。`ultrarender_runtime_candidate.dll` 可动态发现 Runtime/Instance/Error/Operation/Event 表，并实现 typed handle、capability negotiation、retained error、operation/wait/cancel 和有界 event queue；外部 C11 client 不链接 import library。它仍没有 scene/session/frame/renderer/worker 能力，也不是稳定 ABI/Protocol。当前权威游标是 `PB.4 — Immutable frames and Windows local worker`，HR.3 暂缓。架构与施工细则见 [公共 API/ABI 架构](docs/Public_API_ABI_Architecture.md) 和 [Phase PB 计划](docs/PB_Public_Boundary_PLAN.md)；原 Phase X 通用插件生态继续冻结。
...
- 当前 Release 构建注册 85 个 CTest；测试数量只表示已登记门禁规模，不等同于功能覆盖率或发布成熟度。
