...
`PRV.4 — 生产 MeasurementBundle 与输出系统` 已完成；下一权威游标为尚未施工的 `PRV.5 — 训练无关重建与降噪`。PRV.1R 建立了有界原生颜色路径的可信产品 E2E 基线，PRV.2 增加唯一 Scene Realizer、自包含包和统一场景工具扩展，PRV.3 接入 canonical material program set 与受支持资产，PRV.4 将完整场景 typed measurement 和正式输出图接入同一 ProductJob。项目继续冻结 learned/neural、新积分器、广义统一物理世界和可微路线；`UltraRender_preview` 仍是尚未达成的里程碑，不是现有发布版本。
...
PRV.1 已让 CLI render 退出 renderer 实现：共享 `ure_client` 提供显式 Direct/Worker transport，两路均使用同一个 `ure_product` 服务。PRV.1R 明确区分 requested/accepted/completed work，并提供单调进度与最新不可变帧；PRV.3 将 exact-build ProductJob 演进到 0.4，增加计划的 eligible/qualified/executed estimator 观测；PRV.4 增加 exact-build MeasurementFrame/Output 接口而不扩张稳定 Core。CLI 默认 Worker，启动失败不会回退 direct。原有 64×64 smoke 只保留为合同/路由证据；480p functional 与 720p/1080p quality 证据建立了有界产品工作流的 ProductE2E。
...
PRV.4 将 renderer 权威提升为 typed measurement：当前 bounded complete-scene profile 发布 45 个 Beauty/AOV/充分统计/估计器 plane，并通过不可变 Frame lease 在 Direct 与 Worker 中发现和分块读取。正式 artifact graph 使用官方 OpenEXR 3.4.14、Measurement checkpoint v2、原子 manifest 和实际执行 tone-map 的派生 HDR/PPM/BMP；OpenEXR/Imath 静态链接以保持 Core runtime 单 DLL 加载边界。[功能报告](docs/reports/phase_prv4_functional_validation_v1.json)覆盖 854×480、16 spp 的 Direct/Worker、CLI 和外部 SDK，[质量报告](docs/reports/phase_prv4_quality_validation_v1.json)与[视觉审阅](docs/reports/phase_prv4_visual_review_v1.json)覆盖 1280×720、500 spp Cornell。暗部仍有可见 pre-denoise variance；PRV.5 尚未提供生产重建或降噪，Spectrum/Stokes 在缺少真实 producer 时明确拒绝。[Preview 架构](docs/UltraRender_Preview_Architecture.md) 定义目标边界，[PLAN.md](PLAN.md) 从 PRV.5 继续产品总装。
...
PRV.1R 的[功能证据](docs/reports/phase_prv1r_functional_validation_v1.json)覆盖 854×480、16 spp 的 `ure_client` Direct/Worker 与 CLI Direct/Worker；[质量证据](docs/reports/phase_prv1r_quality_validation_v1.json)覆盖 1280×720 Direct 和 1920×1080 Worker 的 128 spp production profile，并保留[视觉审阅](docs/reports/phase_prv1r_visual_review_v1.json)。raw PFM 是权威产物，PNG 使用固定线性 sRGB→Reinhard→sRGB8 查看变换；finite、能量、空间结构和嵌套样本收敛均由门禁检查。PRV.3 扩展了材质/资产/Mie/radiometric-wave 范围，PRV.4 增加 typed measurement/output；这些边界仍不宣称重建、降噪或 Preview 产品完成。
...
最新已冻结的 PB.8 证据为 Release 构建与 101/101 CTest，通过三种独立调用方式生成六幅实际 PFM 图像。PRV.1R 将验证分为 32-128 px contract smoke、480p product functional、720p/1080p product quality 和单独调度的 QHD/UHD stress。当前维护门禁包含可重建的 Cornell 产品 fixture、480p 实际产品调用矩阵和留存质量/视觉证据；PRV.2 增加 procedural self-contained package 的 480p 矩阵；PRV.3 增加材质/资产/波动矩阵；PRV.4 增加 45-plane measurement/artifact graph 的 480p Direct/Worker/CLI/SDK 覆盖和 720p、500 spp Cornell 质量证据。stress 仍按硬件适用性单独调度，不作为普通提交门禁。CTest 数量是构建快照，应以 `ctest -N` 的实时 inventory 为准。
...
- Hydra viewport、legacy Python、生产重建/降噪与非 CUDA complete-scene backend 尚未完成 Preview 收敛；CLI render、scene tooling、bounded glTF/MaterialX/preset authoring 和 typed measurement/output 已共享产品客户端边界，后续语义由 PRV.5-PRV.10 收敛。
