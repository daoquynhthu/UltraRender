...
RenderConfig load_config(const std::string& path) {
    ...
    try {
        ...
        if (j.contains("integrator")) {
            ...
            if (i.contains("target_relative_standard_error")) cfg.integrator.target_relative_standard_error = i["target_relative_standard_error"].get<double>();
            if (i.contains("time_budget_milliseconds")) cfg.integrator.time_budget_milliseconds = i["time_budget_milliseconds"].get<std::uint64_t>();
            if (i.contains("memory_budget_mb")) cfg.integrator.memory_budget_mb = i["memory_budget_mb"].get<int>();
            if (i.contains("pilot_spp")) cfg.integrator.pilot_spp = i["pilot_spp"].get<int>();
            if (i.contains("maximum_techniques")) cfg.integrator.maximum_techniques = i["maximum_techniques"].get<int>();
            if (i.contains("minimum_wavefront_fraction")) cfg.integrator.minimum_wavefront_fraction = i["minimum_wavefront_fraction"].get<double>();
            if (i.contains("allow_experimental")) cfg.integrator.allow_experimental = i["allow_experimental"].get<bool>();
            if (i.contains("sample_index_offset")) cfg.integrator.sample_index_offset = i["sample_index_offset"].get<std::uint64_t>();
...
CliResult parse_cli(int argc, char** argv) {
    ...
    double integrator_target_relative_standard_error = -1.0;
    std::uint64_t integrator_time_budget_milliseconds = 0;
    int integrator_memory_budget_mb = -1;
    int integrator_pilot_spp = -1;
    int integrator_maximum_techniques = -1;
    double integrator_minimum_wavefront_fraction = -1.0;
    bool integrator_allow_experimental = false;
    std::uint64_t integrator_sample_index_offset = 0;
    ...
    render_cmd->add_option("--integrator-mode", integrator_mode, "Compatibility preset: automatic, wavefront, path_guided, restir_di, restir_pt, specular_manifold, bdpt, vcm, mlt");
    ...
    render_cmd->add_option("--target-relative-standard-error", integrator_target_relative_standard_error, "Automatic integration relative standard-error target");
    auto* integrator_time_budget_option = render_cmd->add_option("--integrator-time-budget-ms", integrator_time_budget_milliseconds, "Automatic integration wall-time budget in milliseconds; zero uses the requested SPP budget");
    render_cmd->add_option("--integrator-memory-budget-mb", integrator_memory_budget_mb, "Automatic integration peak memory budget in MiB; zero derives a device budget");
    render_cmd->add_option("--integrator-pilot-spp", integrator_pilot_spp, "Independent pilot samples per candidate technique");
    render_cmd->add_option("--integrator-maximum-techniques", integrator_maximum_techniques, "Maximum automatically selected technique count");
    render_cmd->add_option("--integrator-minimum-wavefront-fraction", integrator_minimum_wavefront_fraction, "Defensive wavefront allocation fraction");
    render_cmd->add_flag("--allow-experimental-integrators", integrator_allow_experimental, "Allow explicitly experimental techniques in automatic qualification");
    auto* integrator_sample_offset_option = render_cmd->add_option("--sample-index-offset", integrator_sample_index_offset, "Global sample identity offset for independent replay or distributed validation");
    ...
    if (*render_cmd) {
        ...
        if (integrator_target_relative_standard_error > 0.0) cfg.integrator.target_relative_standard_error = integrator_target_relative_standard_error;
        if (integrator_time_budget_option->count() > 0) cfg.integrator.time_budget_milliseconds = integrator_time_budget_milliseconds;
        if (integrator_memory_budget_mb >= 0) cfg.integrator.memory_budget_mb = integrator_memory_budget_mb;
        if (integrator_pilot_spp > 0) cfg.integrator.pilot_spp = integrator_pilot_spp;
        if (integrator_maximum_techniques > 0) cfg.integrator.maximum_techniques = integrator_maximum_techniques;
        if (integrator_minimum_wavefront_fraction > 0.0) cfg.integrator.minimum_wavefront_fraction = integrator_minimum_wavefront_fraction;
        if (integrator_allow_experimental) cfg.integrator.allow_experimental = true;
        if (integrator_sample_offset_option->count() > 0) cfg.integrator.sample_index_offset = integrator_sample_index_offset;
