...
bool validate_chain(const void *next, std::uint32_t root_type) noexcept {
    ...
    while (next != nullptr) {
        ...
        if (header.type == 0 || header.type == root_type || header.size < sizeof(header))
            return false;
...
void write_diagnostic(ure_bootstrap_diagnostic_t *diagnostic, ure_result_t result,
                      std::uint32_t detail, std::string_view message) noexcept {
...
ure_result_t fail(ure_bootstrap_diagnostic_t *diagnostic, ure_result_t result, std::uint32_t detail,
...
ure_result_t get_runtime_manifest(const ure_runtime_manifest_request_t *request,
    ...
    if (!digest_is_zero(request->expected_registry_digest) &&
        std::memcmp(request->expected_registry_digest.bytes, registry_digest.data(),
                    registry_digest.size()) != 0) {
        return fail(diagnostic, URE_RESULT_INCOMPATIBLE_VERSION, 5,
                    "registry digest is incompatible");
    }
    ...
    manifest->abi_manifest_json = {reinterpret_cast<const std::uint8_t *>(abi_manifest.data()),
                                   abi_manifest.size()};
    write_diagnostic(diagnostic, URE_RESULT_SUCCESS, 0, {});
    return URE_RESULT_SUCCESS;
}

ure_result_t query_interface(const ure_interface_query_t *query, ure_interface_response_t *response,
    ...
    if (query->reserved[0] != 0 || query->reserved[1] != 0 || response->reserved[0] != 0 ||
    ...
    static constexpr std::array<std::uint8_t, 16> scene_tool_id
    ...
    } else if (uuid_equal(query->interface_id, scene_tool_id)) {
...
extern "C" URE_PUBLIC_API ure_result_t URE_CALL
ureGetRuntimeManifest(const ure_runtime_manifest_request_t *request,
                      ure_runtime_manifest_t *manifest, ure_bootstrap_diagnostic_t *diagnostic) {
    try {
        return get_runtime_manifest(request, manifest, diagnostic);
    } catch (...) {
        if (validate_diagnostic(diagnostic)) {
            return fail(diagnostic, URE_RESULT_INTERNAL, 12,
                        "runtime manifest construction failed");
        }
...
extern "C" URE_PUBLIC_API ure_result_t URE_CALL
ureQueryInterface(const ure_interface_query_t *query, ure_interface_response_t *response,
                  ure_bootstrap_diagnostic_t *diagnostic) {
    try {
