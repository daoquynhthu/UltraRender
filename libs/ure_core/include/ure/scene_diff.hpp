...
struct SceneDiff {
    ...
    std::vector<LegacyMaterialMutation> legacy_materials;
    std::vector<SceneIrInstanceInsertion> scene_ir_instances_to_add;
    std::vector<size_t> scene_ir_instances_to_remove;
    std::vector<LegacyEntityInsertion> legacy_entities_to_add;
    std::vector<size_t> legacy_entities_to_remove;
    std::vector<SceneIrSphereInsertion> scene_ir_spheres_to_add;
    std::vector<size_t> scene_ir_spheres_to_remove;
    std::vector<LegacySphereInsertion> legacy_spheres_to_add;
    std::vector<size_t> legacy_spheres_to_remove;
    ...
    static SceneDiff update_material(size_t material_index, Material material) {
        SceneDiff diff;
        diff.legacy_materials.push_back({material_index, std::move(material)});
        return diff;
    }
    ...
    static SceneDiff add_legacy_entity(RenderEntity entity) {
        SceneDiff diff;
        diff.legacy_entities_to_add.push_back({std::move(entity)});
        return diff;
    }

    static SceneDiff remove_legacy_entity(size_t entity_index) {
        SceneDiff diff;
        diff.legacy_entities_to_remove.push_back(entity_index);
        return diff;
    }
    ...
    static SceneDiff add_legacy_sphere(SphereEntity sphere) {
        SceneDiff diff;
        diff.legacy_spheres_to_add.push_back({std::move(sphere)});
        return diff;
    }

    static SceneDiff remove_legacy_sphere(size_t sphere_index) {
        SceneDiff diff;
        diff.legacy_spheres_to_remove.push_back(sphere_index);
        return diff;
    }
    ...
    bool empty() const {
        return !replacement_scene &&
               ...
               legacy_materials.empty() &&
               scene_ir_instances_to_add.empty() &&
               scene_ir_instances_to_remove.empty() &&
               legacy_entities_to_add.empty() &&
               legacy_entities_to_remove.empty() &&
               scene_ir_spheres_to_add.empty() &&
               scene_ir_spheres_to_remove.empty() &&
               legacy_spheres_to_add.empty() &&
               legacy_spheres_to_remove.empty() &&
               !reset_accumulation;
    }
