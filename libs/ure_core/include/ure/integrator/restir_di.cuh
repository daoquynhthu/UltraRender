...
static __device__ bool compatible_restir_di_sample(
    ...
    if (delta.length_sq() > position_threshold * position_threshold) return false;
    if (domain == GpuRestirDomain::Volume) return true;
    if (source.material_index != material_index || !isfinite(normal_threshold) ||
        normal_threshold < 0.0f || normal_threshold > 1.0f) {
        return false;
    }
    return source.source_normal.dot(normal) >= normal_threshold;
}
