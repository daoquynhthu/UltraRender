...
$LargePattern = [regex]::new(
    'V\.4 large mesh: triangles=(\d+) build_ms=\[([0-9.]+),([0-9.]+),([0-9.]+)\] trace_ms=\[([0-9.]+),([0-9.]+),([0-9.]+)\] node_bytes=\[(\d+),(\d+),(\d+)\].*vram_bytes=(\d+)')
...
$Report = [ordered]@{
    ...
    large_mesh = [ordered]@{
        ...
        compact_bytes = @(
            [uint64]$Large.Groups[8].Value,
            [uint64]$Large.Groups[9].Value,
            [uint64]$Large.Groups[10].Value)
        benchmark_vram_bytes = [uint64]$Large.Groups[11].Value
    }
