...
std::string byte_initializer(std::span<const std::uint8_t> bytes) {
    ...
    for (std::size_t index = 0; index < bytes.size(); ++index) {
        if (index != 0) result << ", ";
        result << "0x" << std::hex << std::setfill('0') << std::setw(2) << static_cast<unsigned>(bytes[index]);
    }
    result << '}';
    return result.str();
}
...
std::vector<std::uint8_t> uuid_bytes(std::string_view uuid) {
    ...
    for (std::size_t index = 0; index < uuid.size();) {
        ...
        result.push_back(static_cast<std::uint8_t>(std::stoul(std::string(uuid.substr(index, 2)), nullptr, 16)));
        index += 2;
    }
    return result;
}
...
std::string registry_header(const Registry& registry) {
    ...
    for (const auto& entry : registry.entries) {
        ...
        if (entry.has_numeric_value) {
            ...
            output << "#define " << entry.c_name << "_UUID_BYTES " << byte_initializer(uuid_bytes(entry.uuid)) << "\n";
        }
    }
    output << "\n#endif\n";
    return output.str();
...
std::string markdown_reference(const Registry& registry) {
    ...
    output << "This generated reference is a Candidate artifact. It is not a stable ABI or protocol promise.\n\n";
    output << "Registry digest: `" << registry.digest_hex << "`\n\n";
    output << "| Registry ID | Kind | Canonical name | Stability | Maturity | Since | Default | Dependencies |\n";
    output << "|---:|---|---|---|---|---|---|---|\n";
    for (const auto& entry : registry.entries) {
...
std::string sha256_file(const std::filesystem::path& path) {
    ...
    return sha256_hex(std::span(reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size()));
}
...
void generate_contract_package(
    ...
    write_text(output_directory / "registry/public_contract_registry.canonical.json", registry.canonical_bytes);
    const std::array schemas{"ure_payload_candidate.fbs", "ure_frame_candidate.fbs", "ure_worker_candidate.fbs"};
    ...
    nlohmann::json manifest{
        ...
        {"canonical_registry", {
            ...
            {"sha256", sha256_file(output_directory / "registry/public_contract_registry.canonical.json")}}},
        {"schema_compiler", "flatc version 25.12.19"},
        {"platform_profiles", {"windows-x64-msvc-c11"}},
        {"public_header_language", "C11"},
        {"worker_protocol", {{"major", 0}, {"minor", 1}, {"maximum_message_bytes", kMaxMockMessageBytes}}},
        {"mock_transport", "bounded fixture framing; production local IPC begins at PB.4"},
    ...
    for (const std::string_view name : schemas) {
        ...
        manifest["schemas"].push_back({{"path", std::string("schemas/") + std::string(name)}, {"sha256", sha256_file(path)}});
    }
    ...
    for (const auto& exchange : build_mock_exchanges(registry)) {
        const auto request_path = output_directory / "golden_messages" / (exchange.name + ".request.bin");
        const auto response_path = output_directory / "golden_messages" / (exchange.name + ".response.bin");
        ...
        scenario_manifest["scenarios"].push_back({
            {"name", exchange.name},
            {"request", std::filesystem::relative(request_path, output_directory).generic_string()},
            {"request_bytes", exchange.request.size()},
            {"request_sha256", sha256_file(request_path)},
            {"response", std::filesystem::relative(response_path, output_directory).generic_string()},
...
void compare_contract_package(
    ...
    try {
        ...
        if (actual_files != expected_files) throw std::runtime_error("Generated file inventory drift");
