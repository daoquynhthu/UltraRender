...
| HO (High-order capabilities) | In progress | HO.0-HO.2, HT.0-HT.4 and HR.0 complete; current cursor: `HT.5 — 自动积分系统闭环` |
...
| **CTest total** | **67 registered tests** in `build_modular_x64` |
...
- Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.4 and HR.0 are complete. The authoritative cursor is `HT.5 — 自动积分系统闭环`; existing CUDA kernels do not yet execute every technique concurrently or populate every high-order measurement plane, and the former Phase X plugin ABI is frozen until the new world/transport/measurement/solver boundaries mature.
