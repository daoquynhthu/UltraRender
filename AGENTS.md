...
The authoritative implementation cursor is `PRV.3 — 材质、资产与有界波动能力组合`; implementation has not started.
...
PRV.1 established this spine for the bounded native-scene color workflow, but its 64×64 smoke evidence was not sufficient ProductE2E proof. PRV.1R repaired sample/work accounting, persistent execution, long-running Worker control, resource roots, hardware applicability and the shared product E2E matrix. PRV.2 added the unique immutable ProductSnapshot Scene Realizer, content-budgeted resources, self-contained `.urepkg`, exact-build Scene Tool 0.1 and a retained 854×480 procedural-package ProductE2E matrix. Hydra and pyure remain explicitly unmerged capability islands; `ultrarender_native_tool` now retains only adapter export. PRV.3 material/asset/wave composition is next and has not begun.
...
| PRV (Preview integration) | PRV.0-PRV.2 done; PRV.3 next, not started | Bounded native color and procedural self-contained-package ProductE2E are trustworthy; material/asset/wave composition remains next |
...
| Public/product boundary | `test_public_boundary_audit`, `test_phase_prv0_baseline`, `test_contract_registry`, `test_public_headers_cpp`, `test_contract_codegen_compare`, `test_contract_schema_conform`, `test_scene_contract_schema_conform`, `test_public_loader_header_mirror`, `test_public_registry_header_mirror`, `test_contract_codegen_negative`, `test_golden_message_mirror`, `test_scene_uuid_golden`, `test_phase_pb6_static`, `test_phase_pb7_static`, `test_phase_pb8_static`, `test_phase_prv1_static`, `test_phase_prv2_static`, `test_mock_worker_external_client`, `test_core_1_0_abi_layout`, `test_core_1_0_loader_exports`, `test_core_1_0_loader_client`, `test_runtime_lifecycle`, `test_frame_leases`, `test_scene_boundary`, `test_scene_transaction`, `test_product_extension`, `test_product_e2e_assets`, `test_product_e2e_evidence`, `test_product_functional_e2e`, `test_client_transport`, `test_scene_tool_extension`, `test_prv1_cli`, `test_prv2_cli`, `test_prv2_package_e2e`, `test_worker_protocol`, `test_worker_crash`, `test_scene_worker`, `test_worker_security`, `test_worker_runtime_security`, `test_pb7_fuzz`, `test_core_1_0_compatibility`, `test_external_client_package` |
...
| **CTest total** | **117 registered tests** in `build_modular_x64` after PRV.2; PB.8 declaration snapshot was 101 |
...
- PRV.0 through PRV.2, Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.8 are complete within their documented product-baseline/component/contract boundaries. PRV.1's former smoke-only ProductE2E conclusion remains superseded; PRV.1R independently closes the bounded native color ProductE2E path, and PRV.2 adds the bounded procedural self-contained-package path. Core ABI 1.0 and Worker Protocol 1.0 are declared only for the documented Windows x64 boundary; there is no UltraRender 1.0 or `UltraRender_preview` product release or public package distribution. The authoritative cursor is PRV.3, which has not started. The approved Preview architecture is `docs/UltraRender_Preview_Architecture.md`; the archived high-order/PB root plan is `docs/archive/High_Order_Public_Boundary_PLAN_2026-08-11.md` and cannot reopen frozen work.
...
- CLI render uses shared `ure_client` Direct/Worker transports and one `ure_product` service with no implicit fallback. Canonical incremental work, resource roots, plan-bound memory applicability, bounded Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive frames/events and the renderer-free exact-build SDK are repaired. Scene tooling uses Scene Tool 0.1 through the same clients; the native tool only exports adapters. Functional 480p, quality 720p/1080p bounded-color evidence and a separate 480p self-contained procedural-package matrix are retained. Hydra and legacy pyure remain product-runtime bypasses scheduled for PRV.10. Accepted-but-ignored semantics are forbidden.
