...
The authoritative implementation cursor is `PRV.5 — 训练无关重建与降噪`; implementation has not started.
...
PRV.1 established this spine for the bounded native-scene color workflow, but its 64×64 smoke evidence was not sufficient ProductE2E proof. PRV.1R repaired sample/work accounting, persistent execution, long-running Worker control, resource roots, hardware applicability and the shared product E2E matrix. PRV.2 added the unique immutable ProductSnapshot Scene Realizer, content-budgeted resources and self-contained `.urepkg`. PRV.3 added canonical material authority, ProductJob 0.4, Scene Tool 0.2, bounded glTF/MaterialX/preset authoring-to-render paths and retained 480p functional plus 720p 500-sample material/Mie/radiometric-wave evidence. PRV.4 added the exact-build MeasurementFrame/Output boundary, 45-plane complete-scene measurements, official OpenEXR/checkpoint/display artifact publication, true partial reads and retained 480p functional plus 720p 500-sample evidence. Hydra and pyure remain explicitly unmerged capability islands; `ultrarender_native_tool` retains only adapter export. PRV.5 reconstruction/denoising is next and has not begun.
...
| PRV (Preview integration) | PRV.0-PRV.4 done; PRV.5 next, not started | Bounded native/package, canonical material/asset/Mie/radiometric-wave and typed measurement/output ProductE2E are trustworthy; reconstruction/denoising remains next |
...
- OSL compiler (URE MaterialGraph supplies canonical topology; the canonical material program set is the product authority, and MaterialX is an adapter)
...
│   ├── ure_native_tool/src/main.cpp # Isolated adapter export; scene tooling policy lives behind the runtime extension
...
| **CTest total** | **125 registered tests** in `build_modular_x64` after PRV.4; PB.8 declaration snapshot was 101 |
...
### 6.1 Subagent Governance

The primary agent may use subagents when delegation has a positive expected return after accounting for coordination, review, shared-worktree risk and total token cost. Delegation is an engineering optimization, not a default ceremony and not a substitute for primary-agent judgment.

- Subagents MUST use the Luna model family with `high` or `xhigh` reasoning. Choose `high` for bounded implementation, inspection and verification; reserve `xhigh` for work whose ambiguity or architectural depth materially benefits from it.
- The primary agent owns task decomposition, dependency ordering, scope control and final acceptance. Every delegated result must be inspected and independently accepted against the authoritative plan and relevant verification contract before it is relied upon.
- Start a subagent only when the expected benefit exceeds doing the work entirely in the primary agent: useful cases include independent bounded work with real parallelism, contained specialist investigation, or a focused audit that reduces material risk. Lower Luna token cost is relevant but does not by itself justify delegation.
- Do not delegate tightly coupled work merely because it is large. Extremely complex, architecture-defining, cross-cutting or shared-state-sensitive work may be safer and cheaper for the primary agent to perform directly.
- Give each subagent a precise deliverable, owned files or read-only scope, dependencies, prohibited areas and acceptance evidence. Avoid overlapping writes. Subagents must not create further agent trees unless the user explicitly requests that topology.
- Do not use subagents for reading AGENTS.md/PLAN.md, summarizing project context, routine command execution, ceremonial duplicate review, or work whose coordination cost dominates execution.
- When waiting for a subagent, do not poll intermediate state. Continue independent primary-agent work when useful; otherwise issue one appropriately long wait for the completed result.
- Stop follow-up turns once the bounded deliverable is obtained. The primary agent remains responsible for integration, regression testing, reporting and commit readiness.
...
### 6.5 Product E2E, Quality, and Scale Evidence Governance

Product evidence is authoritative only when it exercises the maintained product boundary. The canonical route is `ure_client` or a generated SDK, through an explicit Direct or Worker transport, into the runtime Product extension and `ure_product`, ending in a real frame or artifact. CLI is a client adapter over this route, not a separate renderer.

- Direct calls to `ure_core`, `RenderSession`, renderer-private helpers, test-only renderers, mocks, and temporary diagnostic harnesses are component or diagnostic evidence only. They MUST NOT be classified as `ProductE2E`.
- Every maintained calling mode must perform at least one real render and artifact acquisition. Use a risk-based pairwise matrix across Direct, Worker, CLI, external C/C++ SDK clients, and later Python/Hydra adapters; a mock can test contracts but cannot replace a required real path.
- Contract smoke (32-128 px, 1-4 spp), product functional (normally 854×480, 16-64 spp), product quality (1280×720 or 1920×1080, 128-512+ spp), and stress/scale (2560×1440 or 3840×2160, 500+ spp) are separate evidence tiers. Stress is scheduled or manual hardware evidence, not a routine per-commit gate.
- Product quality and stress evidence must use the product-default or explicitly requested production profile. Disabling spectral lanes, estimators, AOVs, reconstruction, precision, or other declared semantics is a labeled diagnostic control and cannot graduate a product capability.
- The renderer's raw artifact remains authoritative. A deterministic derived PNG path must fix orientation, color space, exposure, and tone-map identity. Automated checks cover shape, finite values, nontriviality, energy/statistical bounds, and convergence/reference metrics; quality/release evidence also retains a visual review record. Existence, readable headers, hashes, or transport byte parity alone are insufficient.
- Sample/progress accounting uses explicit requested, accepted, and completed work domains. Pilot and production domains remain separate; scene realization and executor construction must not scale with progress polling or sample count. A phase cannot report completion when requested work was not completed unless an explicitly requested partial-result policy says so.
- Cancellation and deadlines must be observable within a declared bounded work quantum. Worker wait timeouts are client observation windows, not job budgets or automatic failure semantics, and long work must not block cancellation, progress, heartbeat, or shutdown control traffic.
- Evidence binds runtime/build, GPU, driver, VRAM, production profile, resolution, work domain, and timing. Correctness, performance regression, and hardware applicability are distinct judgments. A memory-inapplicable plan must reject or select an explicitly permitted semantically equivalent plan before allocation; silent feature reduction and uncontrolled paging are not valid fallbacks.
- A phase or ledger entry MUST NOT claim `ProductE2E` when its only positive render evidence is smoke-scale, internal, mock-driven, capability-reduced, or does not validate meaningful image content and relevant lifecycle/failure behavior.

### 6.6 Continuous Diagnostic Maturity Governance

Diagnostics are maintained product contracts that mature with every Preview phase. PRV.1R owns the common envelope and minimum behavior; each later phase owns its domain-specific detail schemas, negative E2E, recovery guidance and client mapping. A final release audit cannot substitute for missing diagnostics in the phase that introduced the failure.

- Every applicable failure preserves a stable result/domain and versioned detail plus correlation, operation/session/job and product identities, cause, retryability and recovery hint. Vendor codes are nested typed detail, not the public classification.
- Keep Core results `Timeout`, `Incomplete`, `BudgetExhausted`, `Canceled`, `DeviceLost`, `CapabilityUnavailable` and `Internal` semantically distinct. Express `NotApplicable` and other product states through versioned detail instead of inventing or reinterpreting a Core result. Do not use `Internal` as a catch-all for known resource, input, capability, lifecycle or transport failures.
- Ordinary post-bootstrap failure must expose a retained Error or queryable terminal operation error. Only allocation failure while creating the Error object may omit the handle; it must retain the original result and emit a bounded allocation-free emergency diagnostic.
- Direct, Worker, CLI and maintained SDK/language adapters preserve result/domain/detail/correlation/cause parity. Transport boundaries add context without replacing or flattening the originating error.
- Each production slice that adds or changes a fail-loud path also adds a structured detail owner, negative test, cross-transport evidence, catalog entry and recovery documentation. Otherwise the slice is incomplete.
- Diagnostic payload, cause depth, event rate and retention are budgeted. Messages redact private absolute paths, addresses, credentials and unsafe vendor/untrusted text while retaining content identities and safe field/resource locations needed for repair.
- Device, backend, scene, material, reconstruction, session, distributed and adapter phases extend the shared diagnostic catalog rather than inventing parallel enums or log-only formats.
...
| 7 | 2026-09-07 PRV.3 | Canonical material program set、bounded glTF/MaterialX/preset authoring、material/resource preflight、radiometric-wave ProductE2E 与诊断闭环 | ProductJob 0.4 / Scene Tool 0.2 remain exact-build unstable extensions; five 1280×720 500-sample quality scenes retain visible pre-denoise variance; Direct/Worker parity uses the project 0.5% perceptual bound because adaptive eligible portfolio masks may differ; Release build and 122/122 CTest pass; PRV.4 has not started |
...
| 73 | 2026-08-09 PB.4 | Closed immutable frame leases and the isolated Windows local worker | The 117-entry Candidate registry and generated Frame table provide immutable budgeted snapshots, typed plane identities, checked map/copy leases and explicit backpressure. A same-user, remote-rejecting Named Pipe worker dynamically loads only the two runtime exports, transfers digest-bound read-only mappings, and is contained by a kill-on-close Job Object. Test-only runtime/worker producers are excluded from the product stage; parity, malformed input, crash/restart, import/package and Release 89/89 gates pass. The cursor advances to PB.5 without creating a stable promise or native-scene/render workflow. |
...
| 85 | 2026-08-22 PRV.1R control slice | Established exact work-state and bounded Worker control semantics | Exact-build ProductJob 0.2 preserves requested and accepted domains separately from completed work through Direct, Worker and CLI. Long Worker wait, status probe and cancellation interleave without blocking the control plane; repeated cancellation is terminal-idempotent, one session is serial, and cross-session capacity is explicitly bounded with `Backpressure`. Progressive frames/events, diagnostic/device/sample contracts, external SDK and trusted image evidence remain open. |
| 86 | 2026-08-23 PRV.1R diagnostic/device slice | Established common diagnostics, sample precedence and exact-build device execution semantics | Versioned ErrorDetail and the machine catalog preserve correlation, cause and recovery across Direct/Worker/CLI. Product samples use objective over scene over default without simulation multiplication, ProductJob is single-use, and Device/Execution 0.1 enumerates product adapters, validates stable device constraints before allocation and binds actual execution identity to the plan. Progressive frames/events, external SDK and trusted image evidence remain open. |
| 87 | 2026-08-23 PRV.1R progressive/SDK slice | Established monotonic progressive feedback and renderer-free exact-build integration | ProductJob 0.3 reports coalesced progress, measured remaining ranges and latest immutable frame generations through Direct/Worker without blocking rendering. The staged SDK carries pre-generated protocol headers, generator identity and `UltraRender::Client` CMake examples that build and render out of tree without `flatc` or renderer-private ABI. Functional/quality ProductE2E evidence remains open. |
| 88 | 2026-08-23 PRV.1R closure | Closed trustworthy ProductE2E for the bounded native color workflow | Source-bound Cornell fixtures, a renderer-free shared scenario runner, 854×480 Direct/Worker/CLI functional coverage, 1280×720 Direct and 1920×1080 Worker 128-spp quality evidence, deterministic raw-to-PNG validation and retained visual review passed. Memory planning accounts for queue ping-pong residency and selects the largest plan-bound applicable unbiased candidate subset without reducing declared semantics. Session close now joins the terminal render worker before runtime unload, eliminating the intermittent handle-table teardown race; additive recovery records preserve the historical PRV.1 downgrade while binding the independent PRV.1R evidence. The cursor advances to PRV.2, which has not started. |
| 89 | 2026-08-24 PRV.2 compaction continuation | Recovered the active PRV.2 implementation without reopening unrelated plan sections | Re-read governance and only the PRV.2 authority/dependencies, preserved the dirty implementation, and continued single-agent closure. ProductSnapshot immutability, solver applicability, six resource-budget domains, exact-build Scene Tool routing, diagnostic parity and the procedural-package image path were the retained implementation truths. |
| 90 | 2026-08-24 PRV.2 closure | Closed complete archive realization and self-contained package execution within the bounded supported subset | One realizer executes deterministic procedural archives, resolves content resources, compiles solver/time constraints and freezes ProductSnapshot. Self-contained deterministic packages, Scene Tool 0.1 Direct/Worker/CLI parity, Q.3-Q.12 dispositions, details 600-628, negative fixtures and a source-deleted 854×480 16-spp ProductE2E/visual record passed. Core Frame revision identity was kept distinct from ProductSnapshot identity; Release build and 117/117 CTest passed. Cursor advances to unstarted PRV.3. |
| 91 | 2026-08-24 PRV.3 implementation continuation | Recovered the active material/asset composition implementation without reopening unrelated phases | Canonical material programs, Scene Tool 0.2 MaterialX/preset/glTF authoring, generated exact-build contracts and client/Worker/CLI propagation are in progress. The Cornell glTF authoring fixture and focused Scene Tool/material tests pass; cross-integrator/wave applicability, diagnostics, product image evidence and full validation remain open. |
| 92 | 2026-09-07 PRV.3 closure | Closed bounded canonical material, asset, Mie and radiometric-wave color integration | The canonical material program set binds MaterialGraph topology and all backend-consumed material state; ProductJob 0.4 and Scene Tool 0.2 carry applicability and bounded authoring. Five 480p functional scenes, glTF/preset/MaterialX authoring-to-render paths, five 720p 500-sample quality scenes, visual review, structured failures, Release build and 122/122 CTest passed. Typed measurements, denoising and complete automatic portfolio execution remain PRV.4-PRV.6. |
| 93 | 2026-09-10 PRV.4 closure | Closed bounded complete-scene measurement production and official artifact publication | ProductJob 0.4 publishes 45 typed Beauty/AOV/statistics/estimator planes through exact-build MeasurementFrame/Output interfaces; Direct/Worker/CLI/external SDK share partial reads and one atomic OpenEXR/checkpoint/display/manifest graph. OpenEXR 3.4.14 and Imath 3.2.2 are linked statically to preserve arbitrary-CWD loading by the retained Core 1.0 seed. The 854×480/16-sample matrix and 1280×720/500-sample Cornell quality/visual evidence pass while retaining visible pre-denoise variance; Release build and 125/125 CTest pass. PRV.5 has not started. |

### Consolidated Truth

- The authoritative build tree is `build_modular_x64` using Ninja and the Visual Studio 2026 x64 toolchain.
- PRV.0 through PRV.4, Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.8 are complete within their documented product-baseline/component/contract boundaries. PRV.1's former smoke-only ProductE2E conclusion remains superseded; PRV.1R independently closes the bounded native color path, PRV.2 adds the procedural self-contained-package path, PRV.3 adds bounded canonical material/asset/Mie/radiometric-wave color execution, and PRV.4 adds complete-scene typed measurement/output. Core ABI 1.0 and Worker Protocol 1.0 are declared only for the documented Windows x64 boundary; there is no UltraRender 1.0 or `UltraRender_preview` product release or public package distribution. The authoritative cursor is PRV.5, which has not started. The approved Preview architecture is `docs/UltraRender_Preview_Architecture.md`; the archived high-order/PB root plan is `docs/archive/High_Order_Public_Boundary_PLAN_2026-08-11.md` and cannot reopen frozen work.
...
- CLI render uses shared `ure_client` Direct/Worker transports and one `ure_product` service with no implicit fallback. Canonical incremental work, resource roots, plan-bound memory applicability, bounded Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive frames/events and the renderer-free exact-build SDK are repaired. Scene tooling uses Scene Tool 0.2 through the same clients; ProductJob 0.4 reports material-aware estimator applicability, and the native tool only exports adapters. Exact-build MeasurementFrame/Output exposes 45 typed planes, true partial reads and official atomic artifacts while Core-only clients retain Color fallback. Functional 480p, 720p/1080p bounded-color, procedural-package, PRV.3 material/Mie/radiometric-wave and PRV.4 720p 500-sample measurement evidence are retained. Hydra and legacy pyure remain product-runtime bypasses scheduled for PRV.10. Accepted-but-ignored semantics are forbidden.
