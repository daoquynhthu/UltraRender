...
```text
contracts      — Single source for public IDs, schemas, manifests, compatibility baselines and golden messages (PB.0-PB.6 implemented).
...
ure_contract   — Candidate product runtime DLL with two bootstrap exports and lifecycle/Frame/Scene/Session/transaction tables (PB.6 implemented).
ure_worker     — Candidate local Windows worker that loads the product runtime only through the public loader ABI and applies bounded scene transactions/renders (PB.6 implemented).
```
...
| PB (Public Boundary) | In progress | PB.0-PB.6 complete; current cursor: `PB.7 — Mixed-version, security, packaging, and external-client closure` |
...
| **CTest total** | **95 registered tests** in `build_modular_x64` |
...
- Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.6 are complete. HR.3 is suspended and the authoritative cursor is `PB.7 — Mixed-version, security, packaging, and external-client closure`. The approved architecture and detailed execution authority are `docs/Public_API_ABI_Architecture.md` and `docs/PB_Public_Boundary_PLAN.md`. PB establishes a minimal client boundary, not the former Phase X plugin ecosystem; PB.0-PB.7 remain Candidate 0.x with no stable promise.
