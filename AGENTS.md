...
The authoritative implementation cursor is `PRV.1R — Product Runtime 阻塞性修复与可信 E2E 基线`.
...
PRV.1 established this spine for the bounded native-scene color workflow, but its 64×64 smoke evidence is not sufficient ProductE2E proof. PRV.1R now repairs sample/work accounting, persistent execution, long-running Worker control, resource roots, hardware applicability and the shared product E2E matrix before PRV.2 may proceed. Hydra and pyure remain explicitly unmerged capability islands; native scene tooling is temporarily isolated in `ultrarender_native_tool` until PRV.2 moves its semantic authority behind a runtime extension.
...
| PRV (Preview integration) | PRV.0-PRV.1 structural work done; PRV.1R active | Client spine retained; runtime correctness and trustworthy product E2E must close before PRV.2 complete-scene realization |
...
- PRV.0 and the PRV.1 structural client spine, Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.8 are complete within their documented product-baseline/component/contract boundaries. PRV.1's former ProductE2E conclusion is superseded by the runtime audit. Core ABI 1.0 and Worker Protocol 1.0 are declared only for the documented Windows x64 boundary; there is no UltraRender 1.0 or `UltraRender_preview` product release or public package distribution. The authoritative cursor is PRV.1R, which blocks PRV.2. The approved Preview architecture is `docs/UltraRender_Preview_Architecture.md`; the archived high-order/PB root plan is `docs/archive/High_Order_Public_Boundary_PLAN_2026-08-11.md` and cannot reopen frozen work.
...
- CLI render uses shared `ure_client` Direct/Worker transports and one `ure_product` service with no implicit fallback. Canonical incremental work, resource roots, memory applicability, bounded Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive frames/events and the renderer-free exact-build SDK are repaired, but functional/quality image evidence still prevents a ProductE2E claim. Hydra and legacy pyure remain product-runtime bypasses scheduled for PRV.10; native scene tooling is isolated in `ultrarender_native_tool` until PRV.2 moves its semantic authority behind a runtime extension. Accepted-but-ignored semantics are forbidden.
