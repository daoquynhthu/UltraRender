...
The authoritative implementation cursor is `PRV.2 — 完整场景实现与自包含包`; implementation has not started.
...
PRV.1 established this spine for the bounded native-scene color workflow, but its 64×64 smoke evidence was not sufficient ProductE2E proof. PRV.1R repaired sample/work accounting, persistent execution, long-running Worker control, resource roots, hardware applicability and the shared product E2E matrix. Retained 480p functional and 720p/1080p quality evidence now closes ProductE2E only for that bounded color path. PRV.2 is next and has not begun. Hydra and pyure remain explicitly unmerged capability islands; native scene tooling is temporarily isolated in `ultrarender_native_tool` until PRV.2 moves its semantic authority behind a runtime extension.
...
| PRV (Preview integration) | PRV.0-PRV.1R done; PRV.2 next, not started | Bounded native color ProductE2E is trustworthy; complete-scene realization and self-contained packages remain next |
...
| **CTest total** | **111 registered tests** in `build_modular_x64` after PRV.1R; PB.8 declaration snapshot was 101 |
...
- PRV.0 through PRV.1R, Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.8 are complete within their documented product-baseline/component/contract boundaries. PRV.1's former smoke-only ProductE2E conclusion remains superseded; PRV.1R independently closes the bounded native color ProductE2E path with functional and quality evidence. Core ABI 1.0 and Worker Protocol 1.0 are declared only for the documented Windows x64 boundary; there is no UltraRender 1.0 or `UltraRender_preview` product release or public package distribution. The authoritative cursor is PRV.2, which has not started. The approved Preview architecture is `docs/UltraRender_Preview_Architecture.md`; the archived high-order/PB root plan is `docs/archive/High_Order_Public_Boundary_PLAN_2026-08-11.md` and cannot reopen frozen work.
...
- CLI render uses shared `ure_client` Direct/Worker transports and one `ure_product` service with no implicit fallback. Canonical incremental work, resource roots, plan-bound memory applicability, bounded Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive frames/events and the renderer-free exact-build SDK are repaired. Functional 480p and quality 720p/1080p raw/PNG evidence supports ProductE2E only for the bounded native color workflow. Hydra and legacy pyure remain product-runtime bypasses scheduled for PRV.10; native scene tooling is isolated in `ultrarender_native_tool` until PRV.2 moves its semantic authority behind a runtime extension. Accepted-but-ignored semantics are forbidden.
