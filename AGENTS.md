...
| 12 | 2026-07-18 R-P5 implementation | Enabled production PSSMLT runtime while retaining the R-P5 cursor | Shared queue-owned primary-sample replay, independent GPU chains, bootstrap/burn-in/normalization, diagnostics, memory budgets, 64-bit multi-GPU shard identity, native/config/C/Python propagation, and deterministic GPU E2E are implemented. Reliable small-emitter time-to-error benefit exists; the second difficult-scene benefit remains open, so R-P5 is not closed. |
