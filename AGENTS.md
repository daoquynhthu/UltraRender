...
The authoritative implementation cursor is `PRV.4 — 生产 MeasurementBundle 与输出系统`; implementation has not started.
...
| PRV (Preview integration) | PRV.0-PRV.3 done; PRV.4 next, not started | Bounded native/package and canonical material/asset/Mie/radiometric-wave color ProductE2E are trustworthy; typed MeasurementBundle/output remains next |
...
| **CTest total** | **122 registered tests** in `build_modular_x64` after PRV.3; PB.8 declaration snapshot was 101 |
...
- PRV.0 through PRV.3, Phase Q, Phase M, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2 and PB.0-PB.8 are complete within their documented product-baseline/component/contract boundaries. PRV.1's former smoke-only ProductE2E conclusion remains superseded; PRV.1R independently closes the bounded native color path, PRV.2 adds the procedural self-contained-package path, and PRV.3 adds bounded canonical material/asset/Mie/radiometric-wave color execution. Core ABI 1.0 and Worker Protocol 1.0 are declared only for the documented Windows x64 boundary; there is no UltraRender 1.0 or `UltraRender_preview` product release or public package distribution. The authoritative cursor is PRV.4, which has not started. The approved Preview architecture is `docs/UltraRender_Preview_Architecture.md`; the archived high-order/PB root plan is `docs/archive/High_Order_Public_Boundary_PLAN_2026-08-11.md` and cannot reopen frozen work.
...
- CLI render uses shared `ure_client` Direct/Worker transports and one `ure_product` service with no implicit fallback. Canonical incremental work, resource roots, plan-bound memory applicability, bounded Worker control, common diagnostics, sample precedence, Device/Execution 0.1, monotonic progressive frames/events and the renderer-free exact-build SDK are repaired. Scene tooling uses Scene Tool 0.2 through the same clients; ProductJob 0.4 reports material-aware estimator applicability, and the native tool only exports adapters. Functional 480p, 720p/1080p bounded-color, procedural-package and PRV.3 720p 500-sample material/Mie/radiometric-wave evidence are retained. Hydra and legacy pyure remain product-runtime bypasses scheduled for PRV.10. Accepted-but-ignored semantics are forbidden.
