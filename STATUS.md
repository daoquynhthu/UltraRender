...
The authoritative cursor is `HR.0 — MeasurementBundle / Feature Film`. Phase Q, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2 and HT.0 are complete. `ure_transport` now describes the existing wavefront, guiding, ReSTIR, SMS, BDPT, VCM and PSSMLT routes as a typed Technique Graph compiled from a behavior-preserving legacy preset. This is an executable description boundary, not automatic composition; support/measure composition remains HT.1+ work. The former Phase X plugin ABI remains frozen. Production coherent and partially coherent sessions still reject before GPU allocation.
...
| Registered tests | 60 CTest entries at this snapshot |
