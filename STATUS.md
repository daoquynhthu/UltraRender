...
The authoritative construction cursor is `V.10`. Phase Q, Phase R and Phase T are complete. Phase V has completed V.0 through V.9: the acceleration audit, configuration/API contract, checked CUDA self-compute baseline, TLAS/BLAS split, measured SAH/SBVH wide-node quality presets, bounded asynchronous construction/upload, optional native RT provider construction, one canonical SceneIR cross-provider traversal-parity contract, an SDK-free clustered geometry resource/streaming contract, and shared host/CUDA physical-error LoD selection. Vulkan RT and DXR execute multi-BLAS/TLAS build, compaction, refit/rebuild, scratch budgets and telemetry. OptiX uses the same construction contract and a real OptiX IR raygen/miss/closest-hit pipeline when its separately installed SDK is present, and rejects deterministically when absent. The full SceneIR renderer remains unavailable on native providers. V.8/V.9 provide derived meshlet resources, conservative bounds, material/spectral resource boundaries, multi-component physical LoD error, page residency and path-sensitive selection; the main renderer cluster flag remains fail-loud until V.10 integrates rigid/deforming/topology-change resource lifecycle with SceneDiff.
...
| Registered tests | 52 CTest entries at this snapshot |
...
| GPU geometry acceleration | V.0-V.9 complete | CUDA self-compute, optional OptiX, Vulkan RT and DXR share one canonical traversal fixture. SDK-free clustered geometry defines deterministic meshlets, conservative error-expanded bounds, material/spectral/displacement/opacity/normal-field boundaries, page residency and GPU upload ABI. Shared host/CUDA selection uses ray differential and path/material/resource sensitivity; V.10 owns dynamic SceneDiff lifecycle |
