...
The authoritative construction cursor is `V.5`. Phase Q, Phase R and Phase T are complete. Phase V has completed V.0 through V.4: the acceleration audit, configuration/API contract, checked CUDA self-compute baseline, TLAS/BLAS split and measured SAH/SBVH wide-node quality presets. Provider, quality, update, clustering, statistics and scratch-budget requests have JSON, CLI, C ABI and pyure parity; CUDA self-compute quality presets, static/refit policy and versioned acceleration statistics execute, while unimplemented combinations fail before rendering. Full SceneIR rendering is not yet lowered to Vulkan or D3D12 and remains dependent on the Phase V production acceleration stack.
...
| GPU geometry acceleration | V.0-V.4 complete | CUDA self-compute uses object-space mesh BLAS plus a checked world-space instance TLAS. Auto/fast build use compatible median BVH2; balanced uses binned SAH/BVH4; high quality uses bounded SBVH/BVH8. Quantized wide layouts, original primitive references, checked traversal and C++/C ABI v3/pyure build/memory/work telemetry are measured against the fixed large-mesh gate. V.5 owns async upload, compaction and scratch-budget enforcement |
