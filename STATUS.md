...
The authoritative construction cursor is `V.4`. Phase Q, Phase R and Phase T are complete. Phase V has completed V.0 through V.3: the acceleration audit, configuration/API contract, checked CUDA self-compute baseline and TLAS/BLAS split. Provider, quality, update, clustering, statistics and scratch-budget requests have JSON, CLI, C ABI and pyure parity; CUDA self-compute defaults, static/refit policy and acceleration statistics execute, while unimplemented combinations fail before rendering. Full SceneIR rendering is not yet lowered to Vulkan or D3D12 and remains dependent on the Phase V production acceleration stack.
...
| GPU geometry acceleration | V.0-V.3 complete | CUDA self-compute uses object-space mesh BLAS plus a checked world-space instance TLAS. Transform hot updates refit retained TLAS topology without re-uploading BLAS data; versioned C++/C ABI/pyure stats expose BLAS/TLAS memory, timing and visits. V.4 owns measured SAH/SBVH and compact wide-node quality presets |
