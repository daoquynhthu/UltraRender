...
PRV.2 is complete. The next authoritative implementation cursor is `PRV.3 — 材质、资产与有界波动能力组合`; implementation has not started. PRV.1R supersedes the former smoke-only ProductE2E interpretation with corrected runtime semantics and retained functional/quality evidence; PRV.2 adds complete archive realization, self-contained packages and unified scene tooling without changing the frozen Core 1.0 prefix. PRV.0-PRV.2, Phase Q, R, T, V, the declared bounded scope of W, U, HO.0-HO.2, HT.0-HT.5, HR.0-HR.2, and PB.0-PB.8 remain complete within their documented contract, component, or evidence boundaries. HR.3, neural systems, new estimator research, the broader high-order physical world, and differentiation remain frozen during the Preview route.
...
|---|---|---|
| Core/Worker scene render | ProductE2E for the bounded native color workflow and one 480p self-contained procedural package; ProductSnapshot realization, resource budgets and solver applicability execute before GPU allocation | PRV.3 onward owns complete material/asset, measurement, reconstruction and portable-backend semantics |
| CLI render/tooling | ProductE2E for the bounded Direct/Worker render paths; validate/inspect/build/migrate/pack/unpack/realize use the same runtime scene-tool extension | Adapter authoring and broader semantics still converge in PRV.3-PRV.10 |
...
The current semantic audit covers 25 maintained inputs: 15 reject outside the executable ProductJob 0.3 subset, 8 execute and 2 execute with explicit semantic debt. No maintained input is accepted-but-ignored; the four archive blocks previously preserved only for tooling now enter the PRV.2 realizer's executed/rejected applicability boundary. The retained scenario manifest binds twelve required workflows across eight coverage dimensions. See the historical [`PRV.0 baseline`](docs/PRV0_Product_Truth_Baseline.md), its [machine report](docs/reports/ure_preview_baseline_v1.json), and the historical [PRV.1 validation report](docs/reports/phase_prv1_validation_v1.json); the latter remains valid for routing/smoke parity but is no longer sufficient ProductE2E evidence.
...
| Registry | 221 live identities, 140 reviewed Core identities, 11 pre-release tombstones; ProductJob, Device/Execution and Scene Tool entries are unstable |
| Stable extensions | None initially |
| Unstable extensions | UUID scene transaction, ProductJob 0.3, Device/Execution 0.1 and Scene Tool 0.1 tables; exact registry/runtime identity required |
...
|---|---|---|
| Product client/runtime | Bounded native color ProductE2E plus one self-contained procedural-package ProductE2E through one execution spine, immutable ProductSnapshot realization, content resources, explicit solver applicability, progressive frames and structured scene diagnostics | PRV.3 onward must converge material, measurement, reconstruction, portable execution and remaining clients |
...
- phase-specific structured diagnostics beyond the PRV.1R common envelope and PRV.2 scene/package catalog; later material, output, reconstruction, automatic, session, distributed and adapter failures remain owned by their integration phases;
...
The current configured `build_modular_x64` inventory contains 117 registered tests; this is an inventory count, not a Preview maturity claim.
