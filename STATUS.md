...
|---|---|---|
| Core/Worker scene render | ProductE2E for bounded native/package and material/asset/radiometric-wave color workflows; ProductSnapshot realization, material/resource preflight and applicability execute before GPU allocation | PRV.4 onward owns typed measurement, reconstruction, full automatic portfolio and portable-backend semantics |
| CLI render/tooling | ProductE2E for bounded Direct/Worker render paths; Scene Tool 0.2 authoring produces glTF, preset and MaterialX-derived canonical artifacts that are rendered by the same ProductJob | Hydra/Python migration and broader semantics remain PRV.4-PRV.10 |
...
The current semantic audit covers 25 maintained inputs: 15 reject outside the executable ProductJob 0.4 subset, 8 execute and 2 execute with explicit semantic debt. No maintained input is accepted-but-ignored; archive and material feature declarations enter the realizer's executed/rejected applicability boundary. The retained scenario manifest binds twelve required workflows across eight coverage dimensions; scenarios that require typed spectral/Stokes/time planes remain open for PRV.4 even when their bounded color rendering passed PRV.3. See the historical [`PRV.0 baseline`](docs/PRV0_Product_Truth_Baseline.md), its [machine report](docs/reports/ure_preview_baseline_v1.json), and the historical [PRV.1 validation report](docs/reports/phase_prv1_validation_v1.json); the latter remains valid for routing/smoke parity but is no longer sufficient ProductE2E evidence.
...
| Registry | 224 live identities, 140 reviewed Core identities, 11 pre-release tombstones; ProductJob, Device/Execution and Scene Tool entries are unstable |
...
|---|---|---|
| Product client/runtime | Bounded native/package plus material, asset, Mie and radiometric-wave color ProductE2E through one execution spine; immutable ProductSnapshot, applicability, progressive frames and structured diagnostics | PRV.4 onward must converge typed measurement, reconstruction, full automatic execution, portable execution and remaining clients |
...
| Materials/assets | Canonical MaterialGraph and bounded glTF/MaterialX/preset, texture/SPD/Mie/medium routes execute through Scene Tool/ProductSnapshot/ProductJob with retained Direct/Worker images | MaterialGraph C++ layout remains internal; Hydra viewport is not yet a product client; typed measurement planes remain PRV.4 |
...
- phase-specific structured diagnostics beyond the PRV.1R common envelope, PRV.2 scene/package catalog and PRV.3 material/asset catalog; later output, reconstruction, automatic, session, distributed and adapter failures remain owned by their integration phases;
...
The current configured `build_modular_x64` inventory contains 122 registered tests; this is an inventory count, not a Preview maturity claim.
