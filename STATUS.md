...
The authoritative cursor is `HO.1 — 统一语义与架构合同`. Phase Q, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, and HO.0 are complete. HO.0 now machine-validates 92 explicit capability diagnostics, eight current integrator techniques, ten measurement gaps, eleven state owners, Research Capsule v1 examples and seven benchmark families. HO.1 owns the first backend-neutral observable, measure, time, identity, uncertainty and compatibility contracts. The former Phase X plugin ABI remains frozen. Production coherent and partially coherent sessions still reject before GPU allocation.
