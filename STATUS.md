...
|---|---|---|
| Core/Worker scene render | ClientReachable; canonical incremental work, persistent executors, monotonic progress/latest frames, explicit resource roots, memory preflight, generic multi-plane leases, structured diagnostics and Device/Execution 0.1 are implemented | Meaningful product functional/quality image evidence remains open |
| CLI render | ClientReachable through `ure_client`; Worker is default and Direct is explicit; fixed client-side product deadline removed | Current smoke does not prove product functional/quality evidence |
...
- long-duration progress/backpressure characterization and cancellation bounds at functional/quality scale;
