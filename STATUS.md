...
The authoritative cursor is `HR.1 — 统计重建基线`. Phase Q, Phase R, Phase T, Phase V, the declared bounded scope of Phase W, Phase U, HO.0-HO.2, HT.0-HT.5 and HR.0 are complete. The transport layer now closes objective-driven automatic plans with defensive wavefront coverage and traceable weights, normalization, uncertainty and budgets. The current CUDA bridge independently pilots and sequentially combines complete unbiased endpoint films; it does not yet execute every fine-grained Technique Graph node in one joint dispatch or populate every high-order plane. CLI/JSON and pyure default to automatic objectives, while the low-level C++ default and named manual modes remain compatibility/reproducibility presets. The former Phase X plugin ABI remains frozen. Production coherent and partially coherent sessions still reject before GPU allocation.
...
| Registered tests | 69 CTest entries at this snapshot |
