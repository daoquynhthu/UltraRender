...
The authoritative construction cursor is `V.3`. Phase Q, Phase R and Phase T are complete. Phase V has completed V.0 through V.2: the acceleration audit, configuration/API contract and checked CUDA self-compute baseline. Provider, quality, update, clustering, statistics and scratch-budget requests have JSON, CLI, C ABI and pyure parity; CUDA self-compute defaults, static update declaration and acceleration statistics execute, while unimplemented combinations fail before rendering. Full SceneIR rendering is not yet lowered to Vulkan or D3D12 and remains dependent on the Phase V production acceleration stack.
...
| GPU geometry acceleration | V.0-V.2 complete | `AccelerationConfig` spans C++/JSON/CLI/C ABI/pyure. CUDA self-compute now validates BVH build inputs, shares checked closest/shadow traversal, rejects missing acceleration and stack overflow, and exposes build/traversal statistics. Instance traversal remains linear; V.3 owns TLAS/BLAS separation and transform updates |
