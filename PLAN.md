...
最后更新: 2026-06-16 (Phase R.5 scene spectral-power wavelength proposal)
...
**状态**: 进行中。R.0-R.4 已覆盖 smoke benchmark 入口、active-count wavefront scheduling、queue overflow telemetry、统一采样维度和 sphere light weighted alias sampling；R.5 已接入 CIE-Y task-weighted wavelength proposal 与首版 scene/material spectral-power proposal table，后续继续扩展多 proposal MIS 权重和 per-resource guiding。
