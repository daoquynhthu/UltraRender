...
| Light tree | baseline 已接入，未完整生产化 | GPU resident recursive light tree 已用于 base light sampling，节点携带 bounds，host build 按空间最长轴与能量平衡分群，per-light PMF 提供 O(1) PDF；仍缺 reference-point aware traversal、动态重建成本控制和多场景收益曲线 |
