...
最后更新: 2026-06-16 (Phase R.0-R.6 pre-guiding closure)
...
**状态**: 进行中。R.0-R.6 作为进入 path guiding 前的 radiometric integrator baseline cleanup 已闭环：smoke benchmark 入口、active-count wavefront scheduling、queue overflow telemetry、统一采样维度、sphere light weighted alias sampling、scene/CIE wavelength proposal、narrowband/SPD proposal variance oracle、HG/Rayleigh phase 三元闭合、Mie unsupported gate、Lambertian/cloth/metal/rough dielectric/thin-film energy/PDF oracles 均已有代码和测试/静态审计。R.7 起进入真正的 guiding/reuse/高级路径空间算法；farm dashboard、Nsight 长跑、light tree、非 sphere light、Mie resource、完整跨场景统计验证不再挂在 R.0-R.6 下，分别归 R.7+、K.5/K.6 或 R.12。
