...
**实施进度（2026-08-22）**: PRV.1R.0 的 additive evidence supersession 已落地；规范 production work quantum、持久 candidate executor、独立 pilot/production/realization/executor 计数、预算未完成失败、固定 Worker/CLI deadline 移除、显式资源根、allocation 前显存计划和 Direct/Worker typed memory/resource error 已进入生产路径。Exact-build ProductJob 0.2 与 Worker payload 现在分别携带 requested/accepted/completed work；长任务期间 wait、status probe 与 cancel 可并发交错，重复取消保持终态，单 session 串行且跨 Worker session 默认限一并以 `Backpressure` 拒绝超额请求。Worker 与 `ure_client` 的单-plane 假设已移除，并由共享 lease 的双-plane conformance frame 验证。渐进 latest frame/event、完整诊断信封、设备 extension、采样优先级表、外部 SDK 和 functional/quality E2E 仍未闭环，因此游标保持 PRV.1R。
