...
R-P7 progress（2026-07-22）：`ure.phase_r.industrial_validation.v1` 已聚合 integrator smoke、light sampling、path guiding、ReSTIR PT、specular manifold、独立 BDPT/VCM、MLT 和 volume/Mie 八类 SHA-256 evidence；所有 JSON 子 suite 已拥有版本化 schema。`LocalQuick` 已通过，记录 samples/s、MSE、variance、time-to-error 与线性 spectral-render RGB reconstruction 的 CIE76 色差；farm sample-range/hash contract、Nsight achieved-occupancy/launch/measured peak-VRAM contract 及严格 `Closure` validator 已落地。path guiding/ReSTIR PT 缺失 required runtime state、MLT 与 adaptive scheduler 组合均已有真实 workload fail-loud 边界证据。Nsight Compute 2025.3 管理员采集已取得 17 次真实 kernel launch/occupancy 和 1,643,118,592-byte device-used VRAM delta，转换器同时严格支持 long/wide CSV。旧 `glass_caustic` statistical reference 在修正后的采样合同下确认属于 camera-delta、超出 standalone anchored-SMS/ordinary bidirectional connection support，现作为要求普通图像非零且高级 technique 不重复沉积的边界场景，不再冒充正向 oracle。Cornell、SDS、small-emitter、volume、high-occlusion 和 complex-material 初始曲线均未证明 BDPT/VCM 正 time-to-error，Closure 因此必须失败。R-P7 尚未闭环：BDPT/VCM estimator/performance 收益与 fresh farm long-run 仍是硬门禁。
