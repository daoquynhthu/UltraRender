...
最后更新: 2026-06-16 (Phase R.6 Rayleigh phase oracle closure)
...
| R.6 | BSDF/phase sampling completeness：rough dielectric BTDF、measured conductor、thin-film、volume HG/Rayleigh/Mie candidate 的 sampling/PDF/eval 三元闭合 | 进行中：Henyey-Greenstein volume phase 已有显式 `eval_henyey_greenstein` / `pdf_henyey_greenstein` / `sample_henyey_greenstein_lds_pdf` 三元接口，GPU 测试覆盖 isotropic PDF、reciprocity、数值归一化和 sampler/evaluator PDF 一致性；Rayleigh phase 已有 `eval_rayleigh_phase` / `pdf_rayleigh_phase` / `sample_rayleigh_phase_lds_pdf` closed-form 三元接口，GPU 测试覆盖前后对称、归一化、方向长度和 sampler/evaluator PDF 一致性；volume continuation `last_pdf` 已从 0 改为真实 phase PDF；Lambertian/cloth/metal 已新增 GPU white-furnace/energy oracle、metal BSDF reciprocity oracle 和 Lambertian PDF oracle；rough dielectric reflection/transmission PDF 已有数值积分归一化 oracle，且 white-furnace energy bound 已按 radiance transport scale 分离验证；thin-film 已有多波长/厚度/角度 s/p energy grid oracle。Mie 仍未实现：真实 Mie 需要粒径分布、复折射率、波长参数化或 tabulated phase resource，不能用 HG/Rayleigh 伪装；后续仍需 Mie phase resource/parameterization boundary，以及 R.12 级别的跨场景统计验证 |
