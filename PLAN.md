...
最后更新: 2026-06-16 (Phase R.6 volume phase boundary closure)
...
**状态**: 进行中。R.0-R.4 已覆盖 smoke benchmark 入口、active-count wavefront scheduling、queue overflow telemetry、统一采样维度和 sphere light weighted alias sampling；R.5 已接入 CIE-Y task-weighted wavelength proposal、首版 scene/material spectral-power proposal table 与 scene/CIE mixture PDF；R.6 已开始补齐 BSDF/phase `eval/sample/pdf` 三元闭合，当前 HG volume phase 的显式 PDF 与 continuation `last_pdf` 已接入，Rayleigh phase 的 closed-form eval/pdf/sample 与归一化 oracle 已建立，Mie phase 在缺少真实参数/resource 时已有 unsupported selector gate，Lambertian/cloth/metal 的 white-furnace energy 与 metal reciprocity oracle 已建立，rough dielectric reflection/transmission PDF normalization、rough dielectric white-furnace energy bound 与 thin-film s/p energy grid oracle 已补齐。
...
| R.5 | Spectral MIS and wavelength guiding：在 Phase L sampled wavelength 基础上加入 wavelength proposal families、spectral power/CIE/task-weighted guiding、per-material/resource spectral importance | 进行中：CIE-Y task-weighted wavelength proposal 已进入 GPU raygen；Importance 模式会优先使用由 scene material emission/albedo spectral carriers 构建的 tabulated spectral-power proposal，缺少可分辨光谱结构时回退 CIE-Y；scene proposal 存在时 raygen 从 scene/CIE 50/50 mixture 抽样，并把 `0.5*p_scene(lambda)+0.5*p_CIE(lambda)` 写入 path state，而不是只记录被抽中 proposal 的 PDF；材质 emission 热更新会同步重建 wavelength proposal table；XYZ estimator 继续消费 wavelength PDF；`gpu_test_spectral` 覆盖 CIE-Y、tabulated proposal/PDF 和 scene/CIE mixture PDF，`gpu_test_render` 覆盖 RenderConfig→GpuContext 策略映射、narrow emission SPD proposal 和 material update rebuild。后续仍需 narrowband/SPD oracle、per-texture/resource guiding 和 fluorescence 前置场景 |
| R.6 | BSDF/phase sampling completeness：rough dielectric BTDF、measured conductor、thin-film、volume HG/Rayleigh/Mie candidate 的 sampling/PDF/eval 三元闭合 | 进行中：Henyey-Greenstein volume phase 已有显式 `eval_henyey_greenstein` / `pdf_henyey_greenstein` / `sample_henyey_greenstein_lds_pdf` 三元接口，GPU 测试覆盖 isotropic PDF、reciprocity、数值归一化和 sampler/evaluator PDF 一致性；Rayleigh phase 已有 `eval_rayleigh_phase` / `pdf_rayleigh_phase` / `sample_rayleigh_phase_lds_pdf` closed-form 三元接口，GPU 测试覆盖前后对称、归一化、方向长度和 sampler/evaluator PDF 一致性；`VolumePhaseFunction` selector 已接入默认 HG 生产路径，并对 Mie 返回 unsupported/pdf=0，测试阻止无真实 Mie 参数/resource 时静默降级；volume continuation `last_pdf` 已从 0 改为真实 phase PDF；Lambertian/cloth/metal 已新增 GPU white-furnace/energy oracle、metal BSDF reciprocity oracle 和 Lambertian PDF oracle；rough dielectric reflection/transmission PDF 已有数值积分归一化 oracle，且 white-furnace energy bound 已按 radiance transport scale 分离验证；thin-film 已有多波长/厚度/角度 s/p energy grid oracle。真实 Mie 仍未实现：后续需要粒径分布、复折射率、波长参数化或 tabulated phase resource，以及 R.12 级别的跨场景统计验证 |
...
| M.2 | GPU 编译：节点图 → GpuMaterial + 内核参数 | 进行中：`GpuSceneCompiler` 已支持单 OutputSurface → 单 BSDF，并在 Phase L.9 接入 MaterialGraph expression graph；ConstantColor/ConstantFloat、Texture2D、Add、Multiply、Mix 可作为 albedo/roughness/emission resource expression 在 device 端按 wavelength/UV 评估，不再退回 32-lane packet flatten。MaterialGraph 现有重复 id、坏引用、环检测，坏图 fail-loud；graph 存在时 graph 输出为 authoritative，scalar material texture fields 不参与 GPU 编译；glTF PBR baseColor/roughness/emissive 已自动生成等价 MaterialGraph，并改用 `MaterialGraph::add_node()` builder 自动分配 id；旧文本场景、`scene_io::load_scene()`、`SceneParser` 兼容 shim、`SceneIR -> Scene` compiler、procedural CLI fallback、`IRenderEngine` Scene overload 与旧视觉 smoke 资产均已移除，C/Python file loading 进入 SceneIR；raw in-memory texture mutation 直接失败，后续资源变更必须走 graph/resource 节点；BSDF layering、procedural material nodes、eta/k/ior texture expression slot 和 MaterialX import/export 仍未完成 |
