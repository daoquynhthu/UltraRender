...
| W.4 | Wave propagation operators：统一 `PropagationOperator` 接口，支持 Fraunhofer、Fresnel、angular spectrum、Rayleigh-Sommerfeld/Huygens-Fresnel 的 CPU oracle 与首个 GPU backend | 已启动：W.2 host path 已有 direct Fraunhofer/Fresnel/angular-spectrum CPU oracle 和测试；统一 operator interface、Rayleigh-Sommerfeld/Huygens-Fresnel oracle 与 GPU backend 仍待做 |
