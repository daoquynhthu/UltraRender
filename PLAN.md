...
**实施进度（2026-08-23）**: PRV.1R.0 的 additive evidence supersession 已落地；规范 production work quantum、持久 candidate executor、独立 pilot/production/realization/executor 计数、预算未完成失败、固定 Worker/CLI deadline 移除、显式资源根、allocation 前显存计划和 Direct/Worker typed memory/resource error 已进入生产路径。Exact-build ProductJob 0.2 与 Worker payload 分别携带 requested/accepted/completed work；长任务期间 wait、status probe 与 cancel 可并发交错，重复取消保持终态，单 session 串行且跨 Worker session 默认限一并以 `Backpressure` 拒绝超额请求。Worker 与 `ure_client` 的单-plane 假设已移除，并由共享 lease 的双-plane conformance frame 验证。统一 versioned ErrorDetail、关联/恢复/cause 语义、机器诊断目录、sample precedence、ProductJob single-use 累积规则，以及独立 Device/Execution 0.1 extension 已闭环；Direct/Worker 可枚举产品 adapter、提交稳定设备约束并查询绑定 plan 的实际执行身份。渐进 latest frame/event、外部 SDK 和 functional/quality E2E 仍未闭环，因此游标保持 PRV.1R。
